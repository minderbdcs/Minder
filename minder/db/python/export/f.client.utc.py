...
class xmldata:
	...
	def runXMLData(self, sql, dataset, ftptype, keys):
		" export a dataset to an XML string "	
                ...
                time.tzset()
                utc = pytz.utc
                wk_tz_here = pytz.timezone(wk_out_timezone)
                #wk_tz_here = dateutil.tz.tzlocal())
                #print time.asctime()
                #
			...
			for pos in range(len(data_fields)):
				...
				if str(cur.description[pos][fdb.DESCRIPTION_TYPE_CODE])[7:-2] == "DateTime" and \
					...
					if fielddata.find(".") > -1:
						...
						fielddata = myparms[0] 
                                        # need to adjust this time back by 10 hours
                                        date_string = fielddata
                                        try:
	                                    ...
	                                    print "date format is none"
                                        else:
                                    	    EpochSeconds = time.mktime(time_tuple)
                                            dt = datetime.datetime.fromtimestamp(EpochSeconds)
                                            # need to convert date to UTC
                                            wk_trn_date_aware = dt.replace(tzinfo=wk_tz_here)
                                            print wk_trn_date_aware
                                            wk_trn_date_utc   = wk_trn_date_aware.astimezone(utc)
                                            print wk_trn_date_utc
                                	    #print wk_trn_date_utc,date_format
                                            fielddata = str(wk_trn_date_utc)
                                	    if fielddata.find("+") > -1:
                                		myparms = fielddata.split("+")
                                		fielddata = myparms[0] 
                                	    wk_trn_date_utc = fielddata
                                	    print wk_trn_date_utc,date_format
                                            print fielddata
                                        #
					wk_rowdata = wk_rowdata +  '"' + str(fielddata) + '",'
...
mydata.getXMLString("GRN","export", " ", ",GRN,", " grn.* , 'APCDVIC' as OWNER_ID, 'ME' as WH_ID ")
mydata = None
mydata = xmldata()
