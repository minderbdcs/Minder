...
def sendFile( toHost , toQueue, prnFile, prnBrand ):
	...
	try:
		if os.name == 'nt':
			...
			if queue == None:
				...
				#retvalue = os.system("lp -d " + queue + " " + prnFile)
				...
				if fileExt.lower() == ".pdf" and wkRawBrands.find(prnBrand) > 0 :
					...
					retvalue = os.system("pdf2ps " + prnFile + " - | lp -d " + queue + " " )
				else:
					#os.system("lp -d " + queue + " " + prnFile)
					#retvalue = os.system("lp -d " + queue + " " + prnFile)
					retvalue = os.system("lp  -o raw -d " + queue + " " + prnFile)
	...
	if wk_sendok:
		if os.name == 'nt':
			...
			print "retvalue from print is ", retvalue
			if retvalue != 0:
				wk_sendok = False
	return (wk_sendok )
#####################################################
...
and coalesce(se.equipment_status,'OK') = 'OK'
	...
	if data_fields is None:
		...
		while not data_fields is None:
			...
			if wk_sendok :
				...
				sys.stdout.flush()
				if wk_sendhtml :
					query2 = """update print_requests set request_status = 'EF' where message_id = '%s' """ % message_id
					print query2
				sys.stdout.flush()
				if wk_sendhtml :
					cur2.execute(query2)
...
print "about to register events \n" 
...
while 1:
	...
	if result is None:
		...
		if result['PRN_REQUEST_NQ'] > 0:
			...
			if result['PRN_REQUEST_NQ_END'] > 0:
				conduit.flush()
				#conduit.close()
				break
			else:
