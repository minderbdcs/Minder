...
"""
<title>
getpicklines.py, Version 10.04.05
</title>
<long>
...
if os.name == 'nt':
	logfile = "d:/tmp/getpicklines.log"
else:
	logfile = "/tmp/getpicklines.log"
havelog = 1;
#print "getpicklines log ", logfile
...
tran_data += tran_delim + "<PickingMode>1" + tran_delim + "<WareHouseID>" + tran_wh + tran_delim
...
query5 = """select pick_item.prod_id 
	from pick_order 
	join pick_item on pick_item.pick_order = pick_order.pick_order 
	where pick_order.update_id='%s'
	group by pick_item.prod_id """
#
cur.execute(query5 %(original_message_id))
## get data record
data_fields = cur.fetchone()
#print data_fields
if data_fields is None:
