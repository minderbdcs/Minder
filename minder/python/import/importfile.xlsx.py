...
This 
    ...
    Have to replace "" in generic to use value "UNKNOWN"
<br>
</long>
"""
import sys
...
def checkDate(hop):
	date_string = hop
	if type(hop) == datetime.datetime:
		#is alreay a datetime object
		date_format = "%Y-%m-%d"
		return hop,date_format
...
wk_grn_field = None
wk_generic_field = None
wk_other18_field = None
# read xlsx 
# convert names
#05-09-25
...
#05-11-25
#"SupplierID","DIVISION_ID","GRN","WH_ID","Location","Company ID","SSN_TYPE","SSN_ID","Brand","Model","Generic","Serial #","Legacy ID","Other 5","Other 6","Other 7","Other 8","Other 9","Other 10","Other 11","Other 19","Other 20","Purchase_Price","Other_18","Label Date","PO_ORDER","NOTES"
#
		...
		for line in reader:
			...
			for wk_line_field in line:
				...
				if wk_line == 1:
					if wk_line_field == "Type":
					...
					elif wk_line_field == "SupplierID" :
						wk_line_field = "SUPPLIER_ID"
						buffer.append( wk_line_field)
						if wk_return_id_field is None:
							wk_return_id_field = wk_field_no
						...
						buffer.append( wk_line_field)
						wk_other18_field = wk_field_no
					elif wk_line_field == "Other_18"  :
						wk_line_field = "OTHER18_QTY"
						buffer.append( wk_line_field)
						wk_other18_field = wk_field_no
					...
					elif wk_line_field == "Purchase_Price"  :
						wk_line_field = "PURCHASE_PRICE"
						buffer.append( wk_line_field)
						wk_purchase_price_field = wk_field_no
						...
						buffer.append( wk_line_field)
					elif wk_line_field == "Other 11" :
						wk_line_field = "OTHER11"
						buffer.append( wk_line_field)
					elif wk_line_field == "Damages (should go in NOTES for ease of upload)" :
						wk_line_field = "OTHER37"
						buffer.append( wk_line_field)
					elif wk_line_field == "DIVISION_ID" :
						wk_line_field = "DIVISION_ID"
						buffer.append( wk_line_field)
					elif wk_line_field == "PO_ORDER" :
						wk_line_field = "PO_ORDER"
						buffer.append( wk_line_field)
					elif wk_line_field == "GRN" :
						wk_line_field = "GRN"
						buffer.append( wk_line_field)
						wk_grn_field = wk_field_no
				else:
					...
					if wk_ssn_id_field == wk_field_no:
					...
					elif wk_current_qty_field == wk_field_no and (wk_current_qty_field is not None):
						wk_line_field = "1"
						# set current_qty 1
						...
						wk_other19_field == wk_field_no or wk_other20_field == wk_field_no or \
						wk_generic_field == wk_field_no:
						...
						if wk_line_field is None:
							...
							if wk_line_field == "":
								wk_line_field = "UNKNOWN"
					elif wk_other18_field == wk_field_no: 
						# if string data make ""
						#print "other18 field no",wk_other18_field
						if wk_line_field is None:
							wk_other18_qty_ok = True
							wk_line_field = "0"
							#print "wk_other18_qty_ok = Nonezero"
						else:
							if checkFloat(wk_line_field):
								wk_other18_qty_ok = True
								#print "wk_other18_qty_ok = float"
							else:
								wk_line_field = None
								wk_line_field = "0"
								#print "wk_other18_qty_ok = zero"
					print  wk_line_field
					buffer.append( wk_line_field)
			if wk_line == 1:
				if wk_current_qty_field is None:
					#no "wh - original" field
					wk_line_field = "CURRENT_QTY"
					buffer.append( wk_line_field)
			else:
				if wk_current_qty_field is None:
...
if len(is_other5_missing) > 0:
	...
	wk_insert_stmt = "insert into global_conditions(result,other_no,description) values('O',?,?)"
	wk_other5_log = logfile + "other5.log"
...
try:
		...
		for line in reader:
			...
			if wk_line > 1:
				wk_ssn_id = line[wk_ssn_id_field]
				print wk_ssn_id 
				wk_grn_id = line[wk_grn_field]
				print wk_grn_id 
				if wk_grn_id  is None:
					wk_grn_id = wk_my_grn	
				elif wk_grn_id == "":
					wk_grn_id = wk_my_grn	
				wk_supplier_id = line[wk_return_id_field]
				print wk_supplier_id 
				if wk_supplier_id  is None:
					wk_supplier_id = wk_my_supplier	
				elif wk_supplier_id == "":
					wk_supplier_id = wk_my_supplier	
				...
				cur6.execute(query4, (wk_grn_id, wk_supplier_id , wk_ssn_id))
except Exception as e   :
	exc_type, exc_obj, exc_tb = sys.exc_info()
	sys.exit('file %s,   %s , %s' % (infile, sys.exc_info(), exc_tb.tb_lineno  ))
...
if wk_date is None:
	...
	if len(wk_date) > 1:
		date_string = wk_date[0]
try:
	#cur6.execute(query4, (wk_current_return_id, wk_current_grn_date , wk_my_grn))
	cur6.execute(query4, (wk_current_return_id, date_string , wk_my_grn))
except Exception as e   :
	exc_type, exc_obj, exc_tb = sys.exc_info()
	sys.exit('file %s,   %s , %s' % (infile, sys.exc_info(), exc_tb.tb_lineno  ))
