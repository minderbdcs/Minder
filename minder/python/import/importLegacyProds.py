...
for line in fileinput.input(infile):
	...
	if wk_line == 1:
		...
		# field1 is Company
		# field2  Prod ID
		# field3 is Alternate Company
		# field4 is Alternate Prod ID
		# field5  Description
		# field6  Qty
		# field7  Weight nnn.nnn
		# field8  Active
		# field9  Volume nnnn.nn
		# 	field10  Current Available Qty
		...
		buffer2 = list()
		buffer2.append(buffer[0])
		buffer2.append(buffer[1])
		buffer2.append(buffer[5])
		buffer_keys = list()
		buffer_keys.append(buffer[0])
		buffer_keys.append(buffer[1])
		buffer_data = list()
		buffer_data.append(buffer[5])
		wk_select_stmt = "select onsite_qty from product_stock_status('%s')" % (buffer[1])
		...
		buffer_data.append(mysiteqty)
		buffer_data.append(legacystatus)
		...
		wk_select_stmt = "select 1 from variance where COMPANY_ID = '%s' and PROD_ID = '%s' and RUN_DATE = 'TODAY'" % tuple(buffer_keys)
		wk_update_stmt = "update variance set LEGACY_QTY = '%s',AVAILABLE_QTY = '%s' ,VARIANCE_STATUS = '%s',CREATE_DATE='NOW' where COMPANY_ID = '%s' and PROD_ID = '%s' and RUN_DATE = 'TODAY'" % tuple(buffer_data + buffer_keys)
		...
		tran_delim = "|"
		tran_data = tran_delim + "<Company>" + buffer[0] + tran_delim 
		tran_data +=  "<EANNumber>" + buffer[1] + tran_delim
		tran_data +=  "<AltCompany>" + buffer[2] + tran_delim
		tran_data +=  "<AltEANNumber>" + buffer[3] + tran_delim
		tran_data +=  "<ShortDescription>" + buffer[4] + tran_delim
		tran_data +=  "<Weight>" + buffer[6] + tran_delim
		tran_data +=  "<StockStatus>" + buffer[7] + tran_delim
		tran_data +=  "<Volume>" + buffer[8] + tran_delim
		addtran4("PPA2","P",tran_delim,"BDCS","XX","SSSSSSSSSSS",tran_data,"N")
