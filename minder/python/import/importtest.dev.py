...
if len(sys.argv)>0:
	print "import filename logfilename "
if len(sys.argv)>1:
	print "import file ", sys.argv[1]
	transactionfile = sys.argv[1]
	inn = open(transactionfile,'r')
	innfileopen = "Y"
else:
...
if os.name == 'nt':
        #dsn="d:/asset.rf/database/test.v39.gdb",
	con = kinterbasdb.connect(
                dsn="c:/asset.rf/database/test.v39.gdb",
		user="sysdba",
		password="masterkey")
else:
	con = kinterbasdb.connect(
		dsn="/data/asset.rf/test.v39.gdb",
		user="sysdba",
		password="masterkey")
...
for line in inn.xreadlines():
	...
	mytime = mx.DateTime.Timestamp(int(tran_date[:4]),
		...
		print "failed in call to proc to add record:" + str(e)
		# must write the failed transaction to a file
		if saveinopen == 'N':
			#print os.path.splitext(transactionfile)
			if len(os.path.basename(transactionfile)) < 30:
				newtrans = os.path.splitext(transactionfile)[0]
				savein = open(str(newtrans) + "R.txt" ,'a')
				saveinopen = "Y"
			else:
				saveinopen = "E"
		if saveinopen == "Y":
			savein.write(line)
		if saveinopen == "E":
			print "tried to process too many times - file name length already 30"
		#cur.close()
		cur = con.cursor()
