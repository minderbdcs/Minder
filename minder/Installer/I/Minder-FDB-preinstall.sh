...
TITLE="Minder Series pre-installation validator for ${SERVERTYPE} Server v5.4"
VER="3.1"
LOG="/tmp/Minder-PreInstall-Check-FBserver.log"
TARGETMODE=$1
...
set ${OSALL}
ELVER="$4"
# except on some Oracle kernels at MC
if [ -z "${ELVER}" ] ; then
 ELVER="$2"
fi
...
if [ "${TARGETMODE}" = "TEST" ] ; then
	${ECHO} "Installing as a TEST system only - DO NOT USE in Production" | tee -a ${LOG}
fi
...
# CREATEAUTH = "Y" will create groups and users
...
if [ "${CREATEAUTH}" == "Y" ] ; then
  INS_logSubSection "Correct Supplemental Groups"
#TODO - check Supplemental groups - Warning - not Fatal
#Supplemental group array
...
INS_testDir ${TREE}/fbdb firebird:firebird 770
STATUS=$?
if [ ${STATUS} -eq -1 ]; then
	EXIST_DATA="ER"
fi
INS_testDir ${TREE}/fbext firebird:firebird 770
STATUS=$?
if [ ${STATUS} -eq -1 ]; then
	EXIST_DATA="ER"
fi
INS_testDir ${TREE}/fbtmp firebird:firebird 770
STATUS=$?
if [ ${STATUS} -eq -1 ]; then
	EXIST_DATA="ER"
fi
INS_testDir ${TREE}/logs firebird:bdcs 770
STATUS=$?
if [ ${STATUS} -eq -1 ]; then
	EXIST_DATA="ER"
fi
INS_testDir ${TREE}/minder firebird:firebird 770
STATUS=$?
if [ ${STATUS} -eq -1 ]; then
	EXIST_DATA="ER"
fi
...
if [ ${EXIST_DATA} = "ER" ] ; then
  INS_validateFileSystem ${TREE}
fi

${ECHO}
INS_logSubSection "/backup"
TREE="/backup"
EXIST_BACKUP="UK"
${ECHO} -n "Result: "
if [ -d ${TREE} ] ; then
...
if [ ${EXIST_BACKUP} = "UK" ] ; then
  EXIST_BACKUP="OK"
fi
if [ ${EXIST_BACKUP} = "ER" ] ; then
  INS_validateFileSystem ${TREE}
fi

${ECHO}
INS_validateResult ${EXIST_DATA} "Directory structure: /data"
INS_validateResult ${EXIST_BACKUP} "Directory structure: /backup"
...
INS_validateResult "OK" "Pre-Installation Checks"
