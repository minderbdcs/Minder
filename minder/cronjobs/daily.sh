...
update sys_equip set  current_logged_in = null, current_person = null where current_logged_on < dateadd(hour,-4,cast('NOW' as timestamp)) ;
update sys_equip set  current_logged_in = null, current_person = null where coalesce((select max(trn_date) from transactions_archive t1 where t1.device_id = sys_equip.device_id and t1.trn_date > 'TODAY'),'TODAY') < dateadd(hour,-4,cast('NOW' as timestamp)) ;
