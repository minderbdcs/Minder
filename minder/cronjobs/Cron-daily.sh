...
update issn set prev_wh_id=wh_id,prev_locn_id=locn_id,wh_id='XD',locn_id='00000000' where wh_id<'XA' and current_qty=0 and issn_status='DX'    ;
update issn set prev_wh_id=wh_id,prev_locn_id=locn_id,wh_id='XD',locn_id='00000000',current_qty=0 where wh_id<'XA' and     current_qty is null and issn_status='DX' ;
...
update sys_equip set  current_logged_on = null, current_person = null where coalesce((select max(trn_date) from transactions_archive t1 where t1.device_id = sys_equip.device_id and t1.trn_date > 'TODAY'),'TODAY') < dateadd(hour,-4,cast('NOW' as timestamp)) ;
