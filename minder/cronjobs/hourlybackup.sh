...
if [  ! -z "${ISC_MIRROR_HOST}" ]; then
	if [ -e "${NBACKUPFILEFROM}" ]; then
		...
		#ssh $ISC_MIRROR_HOST -C "date > /backup/mirror/${MDRBPfx}.hourly.trg"
 		THISFILE=${NBACKUPFILEFROM}
		LASTFILE="/data/tmp/mdr.mirror.last.txt"
		#do stat to get size
		THISSIZE=`stat --printf="%s" "$THISFILE"`
		#does lastfile exist
		if [ ! -e "$LASTFILE" ]; then
 			echo "0" > "$LASTFILE"
		fi
		LASTSIZE=`cat  "$LASTFILE"`
		if [ "$THISSIZE" -ne "$LASTSIZE" ]; then
			#save the file size in /data/tmp/mdr.mirror.last.txt
			echo "$THISSIZE" > "$LASTFILE"
			#do copy the file over to minderdr box
			scp ${NBACKUPFILEFROM}  ${ISC_MIRROR_HOST}:/backup/mirror/${NBACKUPFILETO}
			ssh $ISC_MIRROR_HOST -C "date > /backup/mirror/${MDRBPfx}.hourly.trg"
			scp ${NBACKUPFILEFROM}  ${ISC_MIRROR_HOST2}:/backup/mirror/${NBACKUPFILETO}
		fi
