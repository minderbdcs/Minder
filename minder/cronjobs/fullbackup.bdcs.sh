...
if [ ! -z "${ISC_MIRROR_HOST}" ]; then
	...
	if [ -e "${NBACKUPFILE}" ]; then
		...
		sudo -u bdcs bash -c "rsync -avpz ${NBACKUPFILE}  ${ISC_MIRROR_HOST}:/backup/mirror"
		date +"%c.%N" >> /data/minder/backup/backup.nbk.log
...
# only do this on saturday
	...
	then
		...
		sudo -u bdcs bash -c "ssh $ISC_MIRROR_HOST -C \"rm -f /backup/mirror/${MDRBPfx}*.1.nbk\""
		date >> /data/minder/backup/backup.nbk.log
		if [ -e "${NBACKUPFILE2}" ]; then
			#scp ${NBACKUPFILE2}  ${ISC_MIRROR_HOST}:/backup/mirror
			sudo -u bdcs bash -c "scp ${NBACKUPFILE2}  ${ISC_MIRROR_HOST}:/backup/mirror"
		fi
	fi
	if [ -e "${NBACKUPFILE}" ]; then
		if [ $DOW -eq 6 ]
		then
			if [ -e "${NBACKUPFILE2}" ]; then
				...
				sudo -u bdcs bash -c "ssh $ISC_MIRROR_HOST -C \"date > /backup/mirror/${MDRBPfx}.full.trg\""
			fi
		else
			#ssh $ISC_MIRROR_HOST -C "date > /backup/mirror/${MDRBPfx}.daily.trg"
			sudo -u bdcs bash -c "ssh $ISC_MIRROR_HOST -C \"date > /backup/mirror/${MDRBPfx}.daily.trg\""
	...
	if [ -e "${GBACKUPFILEARC}" ]; then
		...
		sudo -u bdcs bash -c "ssh $ISC_MIRROR_HOST -C \"rm -f /backup/mirror/${MDRBarchive}\""
		date >> /data/minder/backup/backup.nbk.log
		#scp ${GBACKUPFILEARC}  ${ISC_MIRROR_HOST}:/backup/mirror
		sudo -u bdcs bash -c "scp ${GBACKUPFILEARC}  ${ISC_MIRROR_HOST}:/backup/mirror"
	fi
fi
