...
//        }
    ...
    public function findValue($sql) {
        ...
        $args = func_get_args();
        //$log->info('args:' . var_export($args,true));
        //$queryMsg = 'Query text: ' . $sql . '; args: ' . implode(', ', array_slice($args, 1));
	$wk_args_slc =  array_slice($args, 1);
	$wk_args_result = array();
	if (is_array($wk_args_slc)) {
		array_walk_recursive($wk_args_slc,function($v) use (&$wk_args_result){ $wk_args_result[] = $v; });
	}
        $queryMsg = 'Query text: ' . $sql . '; args: ' . implode(', ', $wk_args_result);
        $val = null;
        $query = null;
        $params = array();
        $query = ibase_prepare($this->db, $sql);
    ...
    public function updatePickOrder($pickOrder, $mode = 'default' ) {
       ...
       if($mode == 'default'){
        ...
        if ($query) {
            ...
            // here
            // prepareArgs appears to damage the query - 
            // because it does a ibase_free_query
            // so use the base sql
            //$args = $this->prepareArgs($data);
            $args = $this->prepareArgs($sql, $data);
    ...
    */
     public function updatePickItem($pickItem)
    {
        ...
        if ($query !== false) {
            ...
            $args = $this->prepareArgs($sql, $values);
		    array_unshift($args, $query);
            $values = $args;
...
*/
		...
		//$wk_vars = print_r($tmpData, TRUE);
        	//$log->info('tmp2 from prepareArgs:' . $wk_vars . '; ' );
		array_shift($data);
                $tmpData2 = array($query);
		$tmpData = array_merge($tmpData2, array_values($data));
		//$wk_vars = print_r($tmpData, TRUE);
        	//$log->info('tmp2 from merge:' . $wk_vars . '; ' );
                $result = call_user_func_array('ibase_execute', $tmpData);
                if (false === $result) {
                    $errcode = ibase_errcode();
                    $errmsg = ibase_errmsg();
                    ibase_free_query($query);
                    throw new Minder_Exception($errcode . ': ' .  $errmsg . '. Query text: ' . $sql . implode(',', $data));
                }
                $this->pickOrderRecalculate($pickOrder->pickOrder);
                ibase_free_result($result);
                ibase_free_query($query);
