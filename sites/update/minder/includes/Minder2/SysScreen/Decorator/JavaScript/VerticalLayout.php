...
class Minder2_SysScreen_Decorator_JavaScript_VerticalLayout extends Minder2_SysScreen_Decorator_JavaScript_TemplatedElement {
    ...
    protected function _loadDefaultTemplate()
    {
        ...
        foreach ($this->_getElements() as $element) {
            $template .= '<div><span class="minder-element ' . $element['name'] . '">' . $element['name'] . ':Placement</span></div>';
        }
        $template .= '</div>';
