...
//var_dump($wk_report);
            try {
                $report = Minder_Report_Factory::makeReport($wk_report_id);
                if (!empty($task_result->items['REPS_PARAMETER_1_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_1_QUERY_FIELD'] , $task_result->items['REPS_WH_ID'] );
                if (!empty($task_result->items['REPS_PARAMETER_2_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_2_QUERY_FIELD'] , $task_result->items['REPS_PARAMETER_2'] );
                if (!empty($task_result->items['REPS_PARAMETER_3_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_3_QUERY_FIELD'] , $task_result->items['REPS_PARAMETER_3'] );
                if (!empty($task_result->items['REPS_PARAMETER_4_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_4_QUERY_FIELD'] , $task_result->items['REPS_PARAMETER_4'] );
                if (!empty($task_result->items['REPS_PARAMETER_5_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_5_QUERY_FIELD'] , $task_result->items['REPS_PARAMETER_5'] );
                if (!empty($task_result->items['REPS_PARAMETER_6_QUERY_FIELD'])) 
                    $report->setQueryFieldValue($task_result->items['REPS_PARAMETER_6_QUERY_FIELD'] , $task_result->items['REPS_PARAMETER_6'] );

		if (!empty($task_result->items['REPS_WH_ID'])){
                    if($report->getQueryFieldValue("ParamPARAM_WHOUSE_ID") == ""){
                        $report->setQueryFieldValue("ParamPARAM_WHOUSE_ID" , $task_result->items['REPS_WH_ID'] );
                    }
                }

                $pdfImage = $report->getPdfImage();
                // calc folder to put into
                $wk_file_name = $task_result->items['REPS_WH_ID'] . '_' .
                            $task_result->items['REPS_OUT_FOLDER'] . 
                            $task_result->items['REPS_OUT_FILE_NAME'] . 
                            $wk_current_date . "." . $task_result->items['REPS_OUT_FILE_EXT'] ; 
                // save file
                if (false === file_put_contents($wk_file_name, $pdfImage))
                    echo('Cannot write to "' . $wk_file_name . '".');
               // ok have run and saved the pdf
               // now update the reprots_schedule
            } catch (Exception $e) {
                    echo 'Cannot save PDF for WH ' . $task_result->items['REPS_WH_ID'] . ': ' . $e->getMessage();
            }
