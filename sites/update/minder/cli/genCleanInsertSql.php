...
$keys = array(
    'PICK_ITEM_DETAIL' => array('PICK_DETAIL_ID'),
    'PICK_ITEM' => array("PICK_LABEL_NO"),
    'PACK_SSCC' => array("RECORD_ID"),
    'SSN' => array('SSN_ID'),
    'ISSN' => array('SSN_ID'),
);
 ...
 * @return mixed
 */
function genData($total, &$id, $pid, $pi, $packSscc, $ssn, $issn, $data)
{
    while ($total-- > 0) {
        $pid['PICK_DETAIL_ID'] = $id;
        $pid['PICK_LABEL_NO'] = 'D8' . $id;
        $pid['SSN_ID'] = '999' . $id;
        ...
        $packSscc['PS_SSCC'] = "009900000000000" . $id;
...
$data = genData(2, $id, $pid, $pi, $packSscc, $ssn, $issn, $data);

$packSscc['PS_SSCC_STATUS'] = 'CL';
$packSscc['PS_QTY_SHIPPED'] = 3;
$packSscc['PS_SSCC_WEIGHT'] = 0.3;
...
$data = genData(20000, $id, $pid, $pi, $packSscc, $ssn, $issn, $data);

echo "Generating script..." . PHP_EOL;
...
foreach ($triggers as $triggerName) {
    $script .= 'ALTER TRIGGER ' . $triggerName . ' INACTIVE;' . PHP_EOL;
}
 ...
 * @return string
 */
function _createDeleteQuery($table, $keys, $rowData)
{
 ...
 * @return string
 */
function _createInsertQuery($rowData, $table)
{
    $quotedValues = array();

    foreach ($rowData as $fieldValue) {
        $quotedValues[] = is_null($fieldValue) ? 'null' : "'" . $fieldValue . "'";
    }

    return "INSERT INTO " . $table . " (" . implode(', ', array_keys($rowData)) . ") VALUES (" . implode(", ", $quotedValues) . ");";
}

foreach ($data as $table => $rows) {
    echo "Generating delete script for " . $table . "..." . PHP_EOL;
    foreach ($rows as $rowData) {
        $script .= _createDeleteQuery($table, $keys, $rowData) . PHP_EOL;
    }
    echo "Generating insert script for " . $table . "..." . PHP_EOL;
    foreach ($rows as $rowData) {
        $script .= _createInsertQuery($rowData, $table) . PHP_EOL;
    }
}

foreach ($triggers as $triggerName) {
    $script .= 'ALTER TRIGGER ' . $triggerName . ' ACTIVE;' . PHP_EOL;
}

echo "Writing script to file..." . PHP_EOL;

file_put_contents('./data.sql', $script);
