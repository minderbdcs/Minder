...
try {
    echo ManifestBuilder::getInstance()->bootstrap($defaultConfig)->run();
} catch (Zend_Console_Getopt_Exception $e) {
    echo 'Error: ' . $e->getMessage() . PHP_EOL . PHP_EOL;
    echo ManifestBuilder::getInstance()->getUsageMessage();
} catch (Exception $e) {
    echo 'Critical Error: ' . $e->getMessage() . PHP_EOL . PHP_EOL;
    echo $e->getTraceAsString() . PHP_EOL . PHP_EOL;
    ManifestBuilder::getInstance()->getLogger()->crit($e->getMessage());
}
