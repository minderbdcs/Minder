...
class Admin2_TransactionsTestController extends Minder_Controller_Action {
  ...
  /*	$formValues['prod_id'],
		...
		$formValues['order_sub_type'],
		$formValues['trn_class'*/
	...
	if($transaction->getProdId() == "" && $transaction->getCompanyId() == "" && $transaction->getOrderNo() == "" && 	$transaction->getOrderType() == "" &&$transaction->getOrderSubType() == "" && $transaction->getFamily() == "")
	{
        return $this->minder->getInstance()->doTransactionResponse($transaction, 'Y', $input_source, $date, 'MASTER  ');
	}
	else
	{
        return $this->minder->getInstance()->doTransactionResponseV6($transaction, 'Y');
	}
    }
}
