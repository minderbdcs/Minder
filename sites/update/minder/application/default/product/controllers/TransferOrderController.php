                ...
                case 'confirm':
                    ...
                    foreach ($pickOrders as $pickOrder) {
                        if (!$this->minder->pickOrderConfirm($pickOrder)) {
                            $this->addError('Order ' . $pickOrder . ' was not confirmed.');
                        }
                    }
                ...
                case 'approve pick':
                    ...
                    foreach ($pickOrders as $pickOrder) {
                        if (!$this->minder->pickOrderApprovePick($pickOrder)) {
                            $this->addError('Order ' . $pickOrder . ' was not approved.');
                        }
                    }
                    $this->_redirect('transfer-order');
                    break;
                ...
                case 'approve despatch':
                    ...
                    foreach ($pickOrders as $pickOrder) {
                        if (!$this->minder->pickOrderApproveDespatch($pickOrder)) {
                            $this->addError('Order ' . $pickOrder . ' was not dispatched.');
                        }
                    }
                    $this->_redirect('transfer-order');
                    break;
                ...
                case 'hold':
                    ...
                    foreach ($pickOrders as $pickOrder) {
                        if (!$this->minder->pickOrderHold($pickOrder)) {
                            $this->addError('Order ' . $pickOrder . ' was not held.');
                        }
                    }
                    $this->_redirect('transfer-order');
                    break;
...
*/
                ...
                case 'cancel':
                    ...
                    if (empty($reason)) {
                        $this->addError('Please enter a reason for cancelling.');
                    } else {
                        $pickOrders = $this->session->params[$this->_controller]['index']['pick_orders'];
                        foreach ($pickOrders as $pickOrder) {
                            if (!$this->minder->pickOrderCancel($pickOrder, $reason)) {
                                $this->addError('Order ' . $pickOrder . ' was not canceled.');
                            } else {
    ...
    public function linesAction()
    {
        ...
        $this->view->reportButtonList      = $this->minder->getReportButtonLis();                                      
    ...
    public function detailAction()
    {
        ...
        if (!$dataset instanceof MasterTable_DataSet) {
            $this->addError('Invalid table name.');
