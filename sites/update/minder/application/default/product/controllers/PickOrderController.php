...
class PickOrderController extends Minder_Controller_Action
{
	...
	public function indexAction()
	{
        ...
        $this->log->info('action :' . $_POST['action']);
			switch(strtolower($_POST['action'])) {
				...
				case 'allocate orders':
					...
					->goto('allocate-orders', 'pick-order', '');
					break;

				case 'allocate products':
					$this->_redirector = $this->_helper->getHelper('Redirector');
					$this->_redirector->setCode(303)
					->goto('allocate-products', 'pick-order', '');
					break;
...
//                        var_dump($mode, $order_types, $order_modes, $orders, $order_statuses, $order_prioritys, $ids);
                    ...
                    if($addProduct){
                        ...
                        switch($mode){
                            ...
                            case 3:
                                ...
                                $product =  current($this->minder->getProductInfo($name));
                                
                                if(!empty($product)){
                                    
                                    $productCompanyId   =   $product['COMPANY_ID'];
                                  
                                    if($productCompanyId == $pickOrderData->companyId || empty($productCompanyId) || is_null($productCompanyId) ){
                                   
                                        if($this->minder->defaultControlValues['CONFIRM_WITH_NO_PROD'] == 'T') {
                                        
                                            $this->session->params['pick-order']['index']['pick_items'][0]              =   $product;
                                        ...
                                        $this->addError("Product COMPANY_ID do not match order COMPANY_ID");    
                                    }    
                                } else {
                        ...
                        switch($mode){
                            ...
                            case 4:
                                ...
                                if(!empty($nonProduct)){
                                    ...
                                    $nonProductCompanyId   =   $nonProduct['COMPANY_ID'];
                                    if($nonProductCompanyId == $pickOrderData->companyId || empty($nonProductCompanyId) || is_null($nonProductCompanyId)){
                                        ...
                                        $this->addError("Product COMPANY_ID do not mutch order COMPANY_ID");
                                    }    
        ...
        $fieldToFill = array('pick_order',      'contact_name',  'customer_po_wo',
                             'person_id',       'p_first_name',  'company_id',
                             'pick_due_date',   'pick_status',   'created_by',
                             'pick_priority',   'ship_via',      'despatch_location',
                             'd_city',          'd_state',       'd_country',
                             'from_create_date','to_create_date','special_instructions1',
                             'special_instructions2'
                            );
                            
        $this->_fillFormFields(false, $fieldToFill);
		...
		$allowed = array(
                            'PICK_ORDER'            => 'PICK_ORDER.PICK_ORDER LIKE ? AND ',
                            ...
                            'P_FIRST_NAME'          => 'PICK_ORDER.P_FIRST_NAME LIKE ? AND ',
                            'COMPANY_ID'            => 'PICK_ORDER.COMPANY_ID LIKE ? AND ',
                            'PICK_DUE_DATE'         => 'PICK_ORDER.PICK_DUE_DATE = ? AND ',
                            'PICK_STATUS'           => 'PICK_ORDER.PICK_STATUS = ? AND ',
                            'CREATED_BY'            => 'PICK_ORDER.CREATED_BY LIKE ? AND ',
                            'PICK_PRIORITY'         => 'PICK_ORDER.PICK_PRIORITY LIKE ? AND ',
                            'SHIP_VIA'              => 'PICK_ORDER.SHIP_VIA = ? AND ',
                            'DESPATCH_LOCATION'     => 'PICK_ORDER.DESPATCH_LOCATION = ? AND ',
                            'D_CITY'                => 'PICK_ORDER.D_CITY LIKE ? AND ',
                            'D_STATE'               => 'PICK_ORDER.D_STATE LIKE ? AND ',
                            'D_COUNTRY'             => 'PICK_ORDER.D_COUNTRY LIKE ? AND ',
                            'FROM_CREATE_DATE'      => 'PICK_ORDER.PICK_DUE_DATE >= ? AND ',
                            'TO_CREATE_DATE'        => 'PICK_ORDER.RETURN_DATE <= ? AND ',
                            'SPECIAL_INSTRUCTIONS1' => 'PICK_ORDER.SPECIAL_INSTRUCTIONS1 LIKE ? AND ',
                            'SPECIAL_INSTRUCTIONS2' => 'PICK_ORDER.SPECIAL_INSTRUCTIONS2 LIKE ? AND ',
                            'PICK_LABEL_NO'         => 'PICK_ITEM.PICK_LABEL_NO LIKE ? AND '
                            );
			...
			switch(strtolower($_POST['action'])) {
				...
				case 'save':
					...
					if ($pickOrder->save($_POST)) {
						...
						//!empty($_POST['partial_pick_allowed']) ? 'T' : 'F';
	...
	public function linesAction()
	{
		...
		if (!empty($_POST['action1'])) {
			switch (strtolower($_POST['action1'])) {
				...
				case 'delete line':
                    ...
                    $forbiddenStatuses  = explode(',', $this->minder->defaultControlValues['PICK_CANCEL_PK_STATUS']);
                    ...
                    try {
						foreach ($conditions as $id) {
							if (false === ($pickItem = $this->minder->getPickItemById($id))) {
                                ...
                                if(!in_array($pickItem->items['PICK_LINE_STATUS'], $forbiddenStatuses)){
        ...
        //$fieldsData = $this->minder->getUserHeaders('SCN_SORDER', array());
	...
	private function getOrder($soNumber) {
		...
		if ($soap->login()) {
			...
			$this->view->response['message'] = 'SOAP - Can\'t login';
                ...
                if (!$this->minder->updatePickItemStatus($soNumber, 'WP')) {
			$this->addWarning('Order ' . $soNumber . ' was not saved to Work in Progress.');
		}
                $wkRevertStatus = True;
		...
		if(false == $soap->lockSoapTransaction()) {
			...
			$soap->unlockSoapTransaction();
                ...
                if ($wkRevertStatus) {
                	//    revert status back to the previous value where the status is 'WP'
                	if (!$this->minder->updatePickItemStatusBack($soNumber ,'WP' )) {
				$this->addWarning('Order ' . $soNumber . ' was not saved to Previous Status.');
			}
                } else {
                	//    revert status to 'HD' value where the status is 'WP'
                	if (!$this->minder->updatePickItemStatusBack($soNumber, 'WP', 'HD' )) {
				$this->addWarning('Order ' . $soNumber . ' was not saved to Held Status.');
			}
                } 
