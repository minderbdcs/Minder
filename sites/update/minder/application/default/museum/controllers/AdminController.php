...
class AdminController extends Minder_Controller_Action
{
                    ...
                    foreach($personToDelete as $val) {
                        if ($val != 'on') {
                            ...
                            if (false == $currentResult) {
                                $this->view->flashMessenger->addMessage($val . ' delete failed - ' . $this->minder->lastError);
                            } else {
                                $this->view->flashMessenger->addMessage($val . ' - successfully deleted ');
                            }
                        }
                    }
                }
        ...
        switch ($this->getRequest()->getPost('action')) {
            ...
            case 'SAVE & RETURN':
                ...
                if (count($checkPersonObj) > 0 ) {
                    $this->view->flashMessenger->addMessage('Contact with PERSON ID = '
                    ...
                    if (false === $this->minder->personAdd($this->view->personObj)) {
                            $this->view->flashMessenger->addMessage($this->view->personObj->itemds['PERSON_ID'] . ' - not added'
    ...
    public function crudAction()
    {
        ...
        if (!$dataset) {
            $this->view->flash = 'Error while execute SQL';
        ...
        if(!empty($headers)){
            ...
            $this->view->flash = 'No headers for ' . $dataset->table . ' table';
        ...
        switch (strtoupper($this->getRequest()->getPost('action'))) {
            ...
            case 'REPORT: CSV':
                ...
                $this->_exportHelper()->exportTo($this->getRequest()->getPost('action'), $this->view->headers, $data);
                return;
            case 'DELETE':
                if (($records = $this->_request->getPost('records')) && is_array($records)) {
                    ...
                    if ($this->minder->updateMasterTableDataSet($dataset)) {
                        $this->_helper->flashMessenger->addMessage('Record(s) deleted successfully.');
                    } else {
                        $this->_helper->flashMessenger->addMessage($this->minder->lastError);
                    }
                }
            ...
            case 'CLEAR SLOTTING':
                ...
                $this->view->flash = 'Clear Slotting is complete';
            ...
            case 'PRINT COST CENTRE':
                ...
                if(!empty($records)) {
                    ...
                    if($result && !$result->hasErrors()) {
                        $this->view->flash = $count . ' ' . $printLabelHelper->getMessage($action);
                    } 
                } else {
            ...
            case 'REPRINT':
                ...
                if(!empty($records)) {
                   foreach($data as $record) {
                    ...
                    if($result){
                        $this->view->flash = $count . ' label(s) reprinted successfully';    
                    }
                   }
                } else {
                    $this->view->flash = 'Missing data for reprinting';    
                }
            ...
            case 'UPDATE+CSV':
                ...
                foreach($selectedSlottingProducts as $prodId => $locnId) {
                    if($locnId !== '***') {
                        ...
                        if(!$result) {
                            $this->_helper->flashMessenger->addError('Prod Id: ' . $prodId .' Location: ' . $locnId . ' Error while update location or prod_id.');
                        }
                    }
                }
                if($result) {
                   $this->_helper->flashMessenger->addMessage('Updated Data successfull');
                }
    ...
    */
    ...
    protected function _saveData($table, $dataset)
    {
        ...
        if (!$validateResult->hasErrors()) {
            ...
            if ($this->minder->updateMasterTableDataSet($dataset)) {
                switch ($flagReread) {
                    case true:
                        ...
                        $this->view->flashMessenger->addMessage('Record ' . $this->view->row->id . ' updated successfully');
                        $this->_redirector = $this->_helper->getHelper('Redirector');
                    ...
                    default:
                        $this->view->flash = 'Record ' . $this->view->row->id . ' updated successfully';
            ...
            $this->view->flash = $validateResult->errors;
        }
    }
