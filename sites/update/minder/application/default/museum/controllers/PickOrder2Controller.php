...
class PickOrder2Controller extends Minder_Controller_Action {
    ...
    public function indexAction(){
        ...
        $criteria                     = $this->_makeClause($conditions, $allowed, $searchInputs);
        ...
        $this->view->cancelReasons = array_merge(array('' => ''), $this->minder->getOptionsList('CAN_ORDER', 'DESCRIPTION'));
    ...
    public function calcLinesAction() {
        ...
        if (count($pickOrders)) {
            try {
                $this->view->pickItems = $this->minder->getPickItemsByPickOrders($pickOrders, null,null, null, 'PICK_ITEM');
...
//            $this->view->counters['total_displayed'] +=  $pickItem->items['LINE_TOTAL'];
    ...
    public function ajaxGetNewOrderAction(){
        ...
        $this->view->headers        =   current($this->minder->getHeadersForTabList('PICKORDER', array($selectedTab => '')));
        $this->view->editInputs     =   $this->minder->getEditInputs2('PICKORDER');   
        ...
        if($case == 1){
            if($this->minder->defaultControlValues['CONFIRM_WITH_NO_PROD'] == 'T') {
                if (isset($this->session->params['pick-order']['index']['pick_items'])) {
                    try {
                        $result = $this->minder->addPickItem($this->session->params['pick-order']['index']['pick_items']);
