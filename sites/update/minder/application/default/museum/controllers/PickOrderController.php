...
//                        var_dump($mode, $order_types, $order_modes, $orders, $order_statuses, $order_prioritys, $ids);
		...
		$allowed = array(
            ...
            try {
            	$pickOrders                 = $this->view->minder->getPickOrders($criteria, $pageSelector, $showBy);
                $this->view->pickOrders     = $pickOrders['data'];
                $this->session->pickOrders  = $pickOrders['all'];
            } catch (Exception $e) {
	...
	public function linesAction()
	{
		...
		$clause = $this->_makeClause($this->session->conditions[$this->_controller]['index'], $allowed);
	...
	public function addIssnItemsAction()
	{
		...
		if (isset($this->session->params['pick-order']['index']['pick_items'])) {
			try {
				$this->minder->addPickIssnItem($this->session->params['pick-order']['index']['pick_items']);
