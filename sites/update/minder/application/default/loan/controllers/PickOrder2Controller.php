    ...
    public function indexAction(){
        ...
        switch($action) {
            case 'get_order':
                ...
                $this->_selectPickOrder($soNumber);
...
//            $this->view->counters['total_displayed'] +=  $pickItem->items['LINE_TOTAL'];
    ...
    public function testAction() {
        ...
        $filename = tempnam(sys_get_temp_dir(), 'temp_ini.');
        $tmpConfig = array(
            'orders' => $this->_getMenuConfig($this->view->shortcuts)
        );

        $configWriter->write($filename, new Zend_Config($tmpConfig));

        echo file_get_contents($filename);
    }

    protected function _getMenuConfig($root) {
        $result = array();
        $order = 0;
        foreach ($root as $label => $menuItem) {
            $id = str_replace(array('<', '>', '-'), '', str_replace(array(' '), '_', strtolower($label))) ;
            ...
            if (is_array($menuItem)) {
                $result[$id]['pages'] = $this->_getMenuConfig($menuItem);
            } else {
                $htmlRequest = new Zend_Controller_Request_Http('http://minder/');
                $htmlRequest->setRequestUri($menuItem);

                $htmlRequest = $this->getFrontController()->getRouter()->route($htmlRequest);

                $params = $htmlRequest->getParams();

                if (isset($params['module'])) {
                    $result[$id]['module'] = $params['module'];
                    unset($params['module']);
                }

                if (isset($params['controller'])) {
                    $result[$id]['controller'] = $params['controller'];
                    unset($params['controller']);
                }

                if (isset($params['action'])) {
                    $result[$id]['action'] = $params['action'];
                    unset($params['action']);
                }
