...
class AuditProccessingController extends Minder_Controller_Action
{
        ...
        switch ($tableId) {
            case 'ssns':
                ...
                $allowed = array('st_prod_id'      => 'PROD_ID LIKE ? AND ',
                                 ...
                                 'start_date'      => 'ST_AUDIT_DATE >= zerotime(?) AND ',
                                 'end_date'        => 'ST_AUDIT_DATE <= maxtime(?) AND ',
                ...
                /*if($this->minder->isNewDateCalculation() == false){
                    foreach($conditions as $strKey => $strValue){
                        //if(DateTime::createFromFormat('d.m.Y H:i:s', $strValue)!== FALSE  || DateTime::createFromFormat('d.m.Y',$strValue)!==FALSE) {
                        if($this->minder->isValidDate($strValue)) {
                            $conditions[$strKey] = $this->getRequest()->getParam($strKey);
                        }
                    }
                }*/
    ...
    public function loadIssnsAction()
    {
        ...
        if($this->minder->isNewDateCalculation() == false){
            ...
            $datetimet     = $searchConditionsList['start_date'];
            if($datetimet!=''){
                $tz_tot = 'UTC';
                $format = 'Y-m-d h:i:s';
                $dtt = new DateTime($datetimet, new DateTimeZone($tz_from));
                $dtt->setTimeZone(new DateTimeZone($tz_tot));
                $searchConditionsList['start_date']=$dtt->format($format);
            }    

            $datetimet     = $searchConditionsList['end_date'];
            if($datetimet!=''){
                $tz_tot = 'UTC';
                $format = 'Y-m-d h:i:s';
                $dtt = new DateTime($datetimet, new DateTimeZone($tz_from));
                $dtt->setTimeZone(new DateTimeZone($tz_tot));
                $searchConditionsList['end_date']=$dtt->format($format);
            }
        }
