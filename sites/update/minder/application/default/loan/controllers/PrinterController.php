...
class PrinterController extends Zend_Controller_Action {
    ...
    public function printFormFeedAction() {
        
        $deviceid = $this->minder->limitPrinter;
        $p = $this->minder->getPrinter($deviceid, 'Template1.fmt');
        $p->printFormFeed('Template1.prn');
        $p = $this->minder->getPrinter($deviceid, 'Template2.fmt');
        
        
        if (false != ($data['status'] = $p->printFormFeed('Template2.prn'))) {
            $data['message'] = count($lines) . ' label(s) sent to printer ' . $deviceid;
        } else {
            $data['message'] = 'Error occurred at printer '  . $deviceid . "\n";
        }
