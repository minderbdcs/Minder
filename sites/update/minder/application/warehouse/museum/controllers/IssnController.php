...
class Warehouse_IssnController extends Minder_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        switch (strtoupper($this->getRequest()->getPost('action'))) {
            ...
            case 'REPORT: TXT':
                ...
                $response = $this->minder->getIssns($clause);
                $data = array();
    ...
    public function editAction()
    {
        ...
        $this->view->issnObj   = current($this->minder->getIssns(array('ISSN.SSN_ID = ?' => $this->getRequest()->getParam('edit_issn_ssn_id'))));
        ...
        if (count($this->getRequest()->getPost('action')) > 0) {
            //$params = minder_array_merge(array('show_by'    => $this->view->showBy,
            //                                   'start_item' => $this->view->startItem));
            switch ($this->getRequest()->getPost('action')) {
	            case 'PRINT LABEL':
					$id = $conditions['edit_issn_ssn_id'];
					$conditions[$id] = $id;
					$params['act'] = 'act'; 
					$params['cnt'] = 'cnt';
					
					$this->session->conditions['cnt']['act'] = $conditions;
					$this->_redirector = $this->_helper->getHelper('Redirector');
					$this->_redirector->setCode(303)
									  ->goto('issn-print',
											 'printer',
                ...
                case 'SAVE':
                    ...
                    if ($this->getRequest()->getPost('edt_issn_wh_id') != $this->view->issnObj->items['WH_ID'] || $this->getRequest()->getPost('edt_issn_locn_id') != $this->view->issnObj->items['LOCN_ID']) {
                        ...
                        $transaction->objectId  =   $this->view->issnObj->items['SSN_ID'];
                        $transaction->whId      =   $this->getRequest()->getPost('edt_issn_wh_id');
        ...
        $this->view->warehouseList = $this->minder->getWarehouseList();
        ...
        switch (strtolower($action)) {
            default:
               //-- setup conditions
                ...
                $lines = $this->minder->getIssns($clause, $pageselector, $show_by);
                $lines = $lines['data'];
