    ...
    public function editAction()
    {
        ...
        switch (strtoupper($this->getRequest()->getPost('action'))) {
            case 'SAVE & RETURN':
                ...
                $update['create_date']=$dtt->format($format);
    ...
    public function addAction()
    {
        ...
        switch (strtoupper($this->getRequest()->getPost('action'))) {
            case 'SAVE & RETURN':
                ...
                $to_date     = $add_save_array['create_date'];
                $datetimet = $to_date;
                $tz_tot = 'UTC';
                $format = 'Y-m-d h:i:s';
                ...
                $add_save_array['create_date']=$dtt->format($format);

                $tmp->save($add_save_array);
...
//        $this->render('edit');
		...
		if (!empty($val)){
			...
			$sql .=",".$key."='".$val."'";
                   ...
                   case 'HOME_LOCN_ID':
                       if (!empty($val)) {
			   $tmpval1 = trim($val,'[');
			   $tmpval2 = explode(']',$tmpval1);
			   $val = $tmpval2[0];
                           $newHomeLocation =  $this->view->ssnObj->items['HOME_WH_ID']. $val;
	...
	$this->view->warehouseList     = $this->fillList($this->view->ssnObj->items['HOME_WH_ID'],
                                                        "getWarehouseList");
	...
	$this->view->homeWarehouses = $this->minder->getHomeWarehouses();
