...
class Warehouse_ReplaceController extends Minder_Controller_Action
{
	...
	$this->view->pageTitle      = 'Replace Master Data';
        $this->view->masterDataTableList = minder_array_merge( array('' => ''), $this->minder->getOptionsList('RMD_DDNAME', 'DESCRIPTION', true));
    ...
    public function searchAction() {
        ...
        $searchedFields     =   current($this->minder->getOptionsRecordByCode('RMD_DDNAME', $selectedMasterTable));
        ...
        switch($selectedTable){
            case 'COST_CENTRE':
                foreach($beReplaced as $object){
                    ...
                    $ssnList    =   $this->minder->dataGetSsns($clause, 0, 9999);

                    if($ssnList['total'] > 0){
                        foreach($ssnList['data'] as $ssn){
                            
                            $transaction                    =   new $transactionName($ssn['SSN_ID'],$replacedBy,$ssn['WH_ID'].$ssn['LOCN_ID'],$ssn['CURRENT_QTY']);
                           /* $transaction->ssnId          =   $ssn['SSN_ID'];

                            $transaction->costCenter   =   $replacedBy;
			    $transaction->location        = $ssn['WH_ID'].$ssn['LOCN_ID'];
			    $transaction->issnCurrentQty  = $ssn['CURRENT_QTY'];*/
                            
                            if (false === ($result = $this->minder->doTransactionResponse($transaction, 'Y', 'SSBSSKSSS', '', 'MASTER    '))) {
				$this->addError($this->minder->lastError);
                	    } else {
			 	$this->addMessage($transactionName.' '.$result);
            ...
            case 'GENERIC':
                ...
                foreach($beReplaced as $object){
                    ...
                    $ssnList    =   $this->minder->dataGetSsns($clause, 0, 9999);
                    if($beReplacedSsnType == $replacedBySsnType) {
                    	if($ssnList['total'] > 0){
                            foreach($ssnList['data'] as $ssn){
			...
			$this->addError("SSN_TYPE mismatch");
		    }
                }
            ...
            case 'SSN_SUB_TYPE':
                ...
                foreach($beReplaced as $object){
                    ...
                    $ssnList    =   $this->minder->dataGetSsns($clause, 0, 9999);
                    if($beReplacedSsnType == $replacedBySsnType) {
			if($beReplacedSsnGeneric == $replacedBySsnGeneric) {
                    	    if($ssnList['total'] > 0){
                        	foreach($ssnList['data'] as $ssn){
		        ...
		        $this->addError("GENERIC value mismatch");
		    } } else {
		        $this->addError("SSN_TYPE mismatch");
		    }
                }
