...
class Warehouse_IssnController extends Minder_Controller_Action
{
    ...
    {
        $this->minder->getDBLog('IssnController.indexAction start');
        ...
        $allowed =  array('issn_no'         => 'ISSN.SSN_ID LIKE ?',
                          'location_id'     => 'ISSN.LOCN_ID LIKE ?',
                          'product_id'      => 'ISSN.PROD_ID LIKE ?',
                          'status'          => 'ISSN.ISSN_STATUS = ?',
                          'company_id'      => 'ISSN.COMPANY_ID LIKE ?',
                          'original_ssn'    => 'ISSN.ORIGINAL_SSN LIKE ?',
                          'create_date_from'=> 'CREATE_DATE >= ?',
                          'create_date_to'  => 'CREATE_DATE <= ?',
                          'other1'          => 'ISSN.OTHER1 LIKE ?',
                          'other2'          => 'ISSN.OTHER2 LIKE ?', 
                         );

        $conditions = $this->_setupConditions(null, $allowed);
        $clause     = $this->_makeClause($conditions, $allowed);
        
        $this->minder->getDBLog('about to getIssns ');
        ...
        $this->minder->getDBLog('after array search conditions ');
                    ...
                    foreach ($lines as $line) {
                        if (null != $this->getRequest()->getPost('mass_product_id')) {
                            ...
                            If (false == $currentResult) {
                                $this->view->flashMessenger->addMessage($line->items['SSN_ID'] . ' update Product ID failed - ' . $this->minder->lastError);
                            } else {
                                $this->view->flashMessenger->addMessage($line->items['SSN_ID'] . ' - ' . $this->minder->lastError);
                            }
                        }
                        if (null != $this->getRequest()->getPost('mass_company_id')) {
                            ...
                            $currentResult          = $this->minder->doTransactionResponse($transaction);
                            If (false == $currentResult) {
                                $this->view->flashMessenger->addMessage($line->items['SSN_ID'] . ' update Company ID failed - ' . $this->minder->lastError);
                            } else {
                                $this->view->flashMessenger->addMessage($line->items['SSN_ID'] . ' - ' . $this->minder->lastError);
                            }
                        }
                    ...
                    foreach($lines as $line) {
                        if (false !== array_search($line->id, $conditions, true )) {
                           try{
                                ...
                                if(!$result){
                                    $this->view->flashMessenger->addMessage('Error while print label(s)');
                                    break;
                                }             
                           } catch(Exception $ex){
        ...
        $this->minder->getDBLog('after case of action ');
        ...
        $this->minder->getDBLog('after getssn group ');
        ...
        $this->minder->getDBLog('after get prod desc U ');
        ...
        $this->minder->getDBLog('after array combine ');

        $this->view->supplierIdList = minder_array_merge(array('' => ''), $tempArray);
        $this->minder->getDBLog('after minder array merge persons ');

        $tempArray                  = $this->minder->getLocationListFromIssn();
        $this->minder->getDBLog('after getLocationListFromIssn ');
        $this->view->locationIdList = minder_array_merge(array('' => ''), $tempArray);
        $this->minder->getDBLog('after merge 3 ');
        ...
        $this->minder->getDBLog('after array merge 4 ');
        $this->view->brandList = minder_array_merge(array('' => ''), $this->minder->getBrandList());
        $this->minder->getDBLog('after array merge brands ');
        ...
        $this->minder->getDBLog('after array merge 6 ');
        switch (strtoupper($this->getRequest()->getPost('action'))) {
            case 'REPORT: CSV':
            case 'REPORT: XML':
            case 'REPORT: XLS':
        ...
        $this->minder->getDBLog('after case of posted action ');
        $response = $this->minder->getIssns($clause, 'index', $this->view->navigation['pageselector'], $this->view->navigation['show_by']);
        $this->minder->getDBLog('after getIssns 2 ');
        if (is_array($response)) {
        ...
        $this->minder->getDBLog('before touchinf records ');
        for ($i = 1; $i<=ceil($this->view->numRecords/$this->view->navigation['show_by']); $i++) {
            $this->view->pages[] = $i;
        }
        $this->minder->getDBLog('IssnController end of indexAction ');
    }
    ...
    public function editAction()
    {
        ...
        if (count($this->getRequest()->getPost('action')) > 0) {
            ...
            switch ($this->getRequest()->getPost('action')) {
                ...
                $to_date     = $this->getRequest()->getPost('despatched_date');
                $datetimet = $to_date;
                $tz_tot = 'UTC';
                $format = 'Y-m-d h:i:s';
                ...
                $despatched_date_utc=$dtt->format($format);

                    
                    if(!$this->minder->isAdjustIssn && !$this->minder->isAdmin){
                        $this->view->flashMessenger->addMessage('You do not have premissions to update ISSN.');
                    ...
                    if ($editIssnCurrentQty != 'none' && $editIssnCurrentQty != $this->view->issnObj->items['CURRENT_QTY']) {
                        ...
                        if($this->minder->isStockAdjust){
                            ...
                            $this->view->flashMessenger->addMessage('You do not have premissions to update CURRENT_QTY.');
                        }
                    }
                    ...
                    if ($tryToChange == true) {
                        if ($result === true) {
                            $this->view->flashMessenger->addMessage('Record ' . $this->view->issnObj->items['SSN_ID'] . ' updated successfully');
                        } else {
                            $this->view->flashMessenger->addMessage('Record ' . $this->view->issnObj->items['SSN_ID'] . '. Error occured during update');
                        }
                    }
                    break;
    ...
    {
        $this->minder->getDBLog('IssnController.calcAction start ');
        ...
        $this->view->data = $data;
        $this->minder->getDBLog('IssnController.calcAction end ');
    }
