...
class Warehouse_LocationController extends Minder_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $show_by      = $this->session->navigation[$this->_controller][$this->_action]['show_by'];
        $pageselector = $this->session->navigation[$this->_controller][$this->_action]['pageselector'];
        
        $linesToProcess    = $this->minder->getLocationLines($clause, $pageselector, $show_by);
        $this->view->lines = $lines = $linesToProcess['data'];
        ...
        switch (strtoupper($this->getRequest()->getPost('action'))) {
                ...
                if(count($data) > 0){
                   ...
                   if($this->minder->defaultControlValues['GENERATE_LABEL_TEXT'] == 'T'){
                        foreach($data as $location) {
                            ...
                            if(!$result){
                                $this->view->flash = 'Error while print label(s)';
                                break;    
                            }
                       ...
                       foreach($data as $location) {
                            $locationList   =   $this->minder->getLocationForPrint($location['LOCN_ID'],$location['WH_ID']);
            ...
            case 'MASS UPDATE':
                ...
                if (count($fieldsToUpdate) > 0 ) {
                    ...
                    $this->view->lines = $this->minder->getLocationLines($clause, $pageselector, $show_by);
                } else {
        ...
        $this->_postProcessNavigation($linesToProcess);
    ...
    public function calcAction()
    {
        ...
        switch (strtolower($action)) {
            ...
            default:
                ...
                $lines      = $this->minder->getLocationLines($clause, $pageselector, $show_by);
                $lines      = $lines['data'];
                $conditions = $this->_markSelected($lines, $id, $value, $method, 'index');
            break;
        }
