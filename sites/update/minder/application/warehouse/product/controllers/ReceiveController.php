...
class Warehouse_ReceiveController extends Minder_Controller_Action
{
    ...
    public function purchaseAction()
    {
	    ...
	    $conditions['PURCHASE_ORDER.PURCHASE_ORDER'] = $poNumber;
        }
                    ...
                    if (false === ($ssnOriginal = count($grnvpResult) > 1 ? $this->minder->getOriginalSsn($grnvpResult[0]) : $this->minder->getLastSsnforGrn($grnNo))) { 
                        ...
                        if ($thirdParty) {
                            ...
                            if (count($grnvpResult) > 1) {
                                // Frank Leih 25.08.2010 when generate label text is F so 3rd party
...
*/
	...
	$log->info(' Receive Controller get order ');
	...
	$log->info(' check soap is locked' );
        if(false == $soap->lockSoapTransaction()) {
	    $log->info(' failed to lock soap ' );
            $this->view->response['status'] = false;
	    ...
	    $log->info(' soap is UNlocked' );
	        ...
	        $log->info(' soap - order not found 1' );
                ...
                return;
	                ...
	                $log->info(' got soap response' . $xmlResp );
                        $obj = $parser->parseSearch($xmlResp);                               
                        if(count($obj) == 0) {
	                    $log->info(' soap - order not found 2' );
                            $this->view->response['status'] = false;
                            $this->view->response['message'] = 'Order not found ';
                        } else {
	                            $log->info(' soap - try to importData' );
                                    if($syn->importData($obj, false) == 0) {
	                                $log->info(' soap - order not passed by filter' );
                                        $this->view->response['status'] = false;
                                        $this->view->response['message'] = 'Order not passed by a filter' . $syn->lastError;

                                    } else {
	                                        $log->info(' soap - order added or updated' );
                				...
                				//$this->_redirect('warehouse/receive/order-lines/id/' . $poNumber . '/?fmt=12-inch');
                                    }
                        }

            }
            $soap->unlockSoapTransaction();
	    $log->info(' soap - unlocked' );
        }

	    //$log->info(' about to render get-order' );
            $this->render('get-order');
	    $log->info(' end getorder' );
            return;
    }
