            ...
            if ($selectedOrdersCount > 0) {
                ...
                if ($this->view->selectedLinesCount > 0) {
                    ...
                    if (count($nonOPStatus) > 0)
                        $this->view->warnings[] = 'Cannot allocate lines with status ("' . implode('", "', $nonOPStatus) . '")';
                }
        ...
        try {
            ...
            foreach ($selectedOrders as $order) {
                if (!$this->minder->pickOrderHold($order['PICK_ORDER'])) {
                    $flashMessenger->setNamespace('warnings')->addMessage('Order ' . $order['PICK_ORDER'] . ' was not held.');
                } else {
        ...
        try {
            ...
            foreach ($selectedOrders as $order) {
                if (!$this->minder->pickOrderCancel($order['PICK_ORDER'], $reason)) {
                    $flashMessenger->setNamespace('warnings')->addMessage('Order ' . $order['PICK_ORDER'] . ' was not canceled.');
                } else {
