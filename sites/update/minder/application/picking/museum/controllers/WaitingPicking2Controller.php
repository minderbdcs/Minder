...
class Picking_WaitingPicking2Controller extends Minder_Controller_Action_Picking {
    ...
    public function indexAction() {
        ...
        try {
            ...
            $this->addWarning($e->getMessage() . ' Device Statistics dialog will not work.');
        }
    }
        ...
        try {
            ...
            foreach ($selectedOrders as $order) {
                if (!$this->minder->pickOrderHold($order['PICK_ORDER'])) {
                    ...
                    $this->view->warnings[] = 'Order ' . $order['PICK_ORDER'] . ' was held successfully.';
                }
            }
        } catch (Exception $e) {
            $this->view->errors[] = $e->getMessage();
        ...
        try {
            ...
            foreach ($selectedOrders as $rowId => $row) {
                ...
                if ($this->minder->updatePickOrderPickPriority($order->pickOrder, $order->pickPriority)) {
                    ...
                    $response->warnings[] = 'Pick Order #' . $row['PICK_ORDER'] . ' priority was not updated. ' . $this->minder->lastError;
                }
            }
        } catch (Exception $e) {
