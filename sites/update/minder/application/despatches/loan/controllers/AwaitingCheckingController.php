...
/************************************************/
    ...
    //Function to add procedure call
   ...
   public function getProductAction() {
	...
	$collect = array();
	$collect['product'] = $prodid;
	$collect['pick_order'] = $this->session->pick_order_session;
	$collect['price'] = 0;
	$collect['required_qty'] = 1;
        //$collect['checkin_qty'] = 1;
	$collect['case'] = 2;

	
        $check=$this->minder->getProdLine($prodid); //check prod_type
        
        if($check=="PM")
	{
		$prodstatus=$this->minder->getProdStatus(); //check prod_status
		if($prodstatus=='T')
		{
			
			$result = $this->minder->addPickItem($collect);
			echo $result;
		}
