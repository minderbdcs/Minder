...
//                    $ordercheckModel->movePickItemsToDespatchLocation($whId . $locnId, $searchFields);
            ...
            if ($totalRows > 0) {
                ...
                if (strtoupper($postAction) == 'SEARCH') {
                    $this->view->warnings[] = 'No orders found.';
                }
            }
...
//                    $rowsModel    = $screenBuilder->buildSysScreenModel(self::$linesModel);
            ...
            if ($selectionNamespace == $this->view->linesSelectionNamespace) {
                ...
                if (count($itemsWithZeroQty) > 0) {
                    $response->warnings[] = 'Item(s) ' . implode(', ', $itemsWithZeroQty) . ' have Picked Qty = 0.';
                }
            ...
            if ($selectedOrdersCount > 0) {
                ...
                foreach ($pickOrders->PICK_STATUS as $status) {
                    if ($status != 'DA') {
                        ...
                        $this->view->warnings[] = 'Bad selected Order status.';
                    }
                }
                ...
                if (count($itemsWithBadWh) > 0) {
                    $this->view->warnings[] = 'Item(s) ' . implode(', ', $itemsWithBadWh) . ' have wrong Warehouse.';
                }
        ...
        try {
            ...
            if ($selectedOrdersCount > 0) {
                ...
                if (empty($shipVia) || !isset($this->view->carriers[$shipVia])) {
                    $this->view->warnings[] = empty($shipVia) ? 'PICK_ORDER #' . $selectedOrderNo . ' has empty SHIP_VIA. Will use order company DEFAULT_CARRIER_ID: "' . $orderCompany->DEFAULT_CARRIER_ID . '".' : 'PICK_ORDER #' . $selectedOrderNo . ' SHIP_VIA field contains unknown CARRIER_ID: "' . $shipVia . '". Will use order company DEFAULT_CARRIER_ID: "' . $orderCompany->DEFAULT_CARRIER_ID . '" instead.';
                    $shipVia = $orderCompany->DEFAULT_CARRIER_ID;
                }

                if (empty($shipVia) || !isset($this->view->carriers[$shipVia])) {
                    $this->view->warnings[] = empty($shipVia) ? 'Company #' . $selectedOrder->companyId . ' has empty DEFAULT_CARRIER_ID. Will use CONTROL.DEFAULT_CARRIER_ID: "' . $this->minder->defaultControlValues['DEFAULT_CARRIER_ID'] . '".' : 'Company #' . $selectedOrder->companyId . ' DEFAULT_CARRIER_ID field contains unknown CARRIER_ID: "' . $shipVia . '". Will use CONTROL.DEFAULT_CARRIER_ID: "' . $this->minder->defaultControlValues['DEFAULT_CARRIER_ID'] . '" instead.';
                    $shipVia = $this->minder->defaultControlValues['DEFAULT_CARRIER_ID'];
                }

                if (empty($shipVia) || !isset($this->view->carriers[$shipVia])) {
                    $firstCarrier = current($this->view->carriers);
                    $this->view->warnings[] = empty($shipVia) ? 'DEFAULT_CARRIER_ID is empty. Will use first CARRIER_ID: "' . $firstCarrier . '" from CARRIER table.' : 'DEFAULT_CARRIER_ID contains unknown CARRIER_ID: "' . $shipVia . '". Will use first CARRIER_ID: "' . $firstCarrier . '" from CARRIER table instead.';
        ...
        if (count($pickOrders) > 0) {
            try {
                ...
                if (!empty($nextNotPrintedSscc)) {
                    ...
                    $response->addWarnings('No not printed labels found.');
                }
                $response->orderStatistics = $this->_awaitingCheckingEdiHelper()->calculateEdiOrderStatistics($pickOrders, $dcNo);
            } catch (Exception $e) {
                $response->addErrors($e->getMessage());
    ...
    public function printSsccLabelAction() {
        ...
        if ($totalPrinted > 0) {
            ...
            $this->view->warnings[] = 'No label was printed.';
        }
