    ...
    public function searchAction() {
        ...
        try{
            ...
            if (!empty($searchFields)) {
                ...
                //echo"<pre>"; die(print_r($rowsModel->makeConditionsFromSearch($searchFields)));
...
; // want the order no for the issns pick_order
		...
		// was using a v3 transaction
                ...
                if (false === ($result = $this->minder->doTransactionResponseV6($transaction ))) {
                    ...
                    $this->correctReturnData();
                    $response->messages[]    =   $result;
                }
            }
            else {
                $response->errors[]   =   'Please, select items !';
            }
        }
    ...
    // function to correct data issues after returning
    public function correctReturnData(){
        $arrSelectedItems    =   $this->getRequest()->getParam('selectedItems');

        if(count($arrSelectedItems) != 0){
            $strQuery = "select distinct(PICK_ORDER) from PICK_ITEM where PICK_LABEL_NO in ('".implode($arrSelectedItems, "','")."')";   // "','D389930".
            $rstPickOrder = ibase_query($strQuery);
            while($arrPickOrder = ibase_fetch_assoc($rstPickOrder)){

                $strQuery = "select 
                            PI.*,
                            I.PROD_ID as ISSN_PROD_ID,
                            O.DESCRIPTION as RETURN_DESCRIPTION
                            from PICK_ITEM PI
                            left join OPTIONS O 
                            on O.GROUP_CODE = 'RET_REASON' and PI.REASON = O.CODE
                            left join ISSN I
                            on PI.SSN_ID = I.SSN_ID
                            where 
                            PI.PICK_LINE_STATUS = 'CR' and PI.PICK_ORDER = '".$arrPickOrder["PICK_ORDER"]."' and 
                            CAST(PI.PICK_ORDER_LINE_NO as int) 
                            in(select PICK_ORDER_LINE_NO from PICK_ITEM where PICK_ORDER = '".$arrPickOrder["PICK_ORDER"]."' and PICK_LINE_STATUS = 'DX')";
                $rstOrderDetail = ibase_query($strQuery);
                while($arrOrderDetail = ibase_fetch_assoc($rstOrderDetail)){
                    //print_r($arrOrderDetail);

                    $strLocinId = ltrim($arrOrderDetail["DESPATCH_LOCATION"], $arrOrderDetail["WH_ID"]);
                    $strUpdateIssnQuery = "update issn set
                                            LOCN_ID = '$strLocinId'
                                            where ssn_id = '".$arrOrderDetail["SSN_ID"]."'";
                    ibase_query($strUpdateIssnQuery);

                    $strUpdatePickItemQuery = "update pick_item set
                                            prod_id = '".$arrOrderDetail["ISSN_PROD_ID"]."',
                                            description = '".$arrOrderDetail["RETURN_DESCRIPTION"]."'
                                            where pick_label_no = '".$arrOrderDetail["PICK_LABEL_NO"]."'";
                    ibase_query($strUpdatePickItemQuery);
                }

            }
            ibase_free_result($rstPickOrder);
            ibase_free_result($rstOrderDetail);
            //echo "<pre>"; die(print_r($arrSelectedItems)."***");
        }
