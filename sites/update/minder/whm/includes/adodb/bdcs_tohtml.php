...
function rs2html(&$rs,$ztabhtml=false,$zheaderarray=false,$htmlspecialchars=true,$echo = true,$checkme = false)
{
...
GLOBAL $gSQLMaxRows,$gSQLBlockRows,$ADODB_ROUND;
	...
	for ($i=0; $i < $ncols; $i++) {	
		...
		//$hdr .= "<TH>$fname</TH>";
		$hdr .= "<TH>" . ucwords(strtolower($fname)) . "</TH>";
	}
	$hdr .= "\n</tr>";
	if ($echo) print $hdr."\n\n";
	else $html = $hdr;
	...
	while (!$rs->EOF) {
		...
		$s .= "<TR valign=top>\n";
		if ($checkme) 	
 			$s .= "<TD><input type=\"checkbox\" name=\"reserveme[]\" value=\"" . $rs->fields[0] . "\" onfocus=\"saveMe('" . $rs->fields[0] . "');\"></TD>\n";
