...
//	echo(' alt="' . $alt . '">');
	...
	echo("<input type=\"hidden\" name=\"from\" value=\"getverifyLP.php\" >\n");
...
echo("<input type=\"hidden\" name=\"retfrom\" ");
$Query = "select first 1  person_id, first_name, last_name from person where person_type in ('RP','CO','CS','CV','CR','IS') and status = 'CU'  order by first_name, last_name "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
...
//	echo ("<tr>");
	...
	//echo("<form action=\"getverifyLPweight.php\"  method=\"post\" name=getweight>\n");
/*
	//echo("<input type=\"IMAGE\" ");  
	echo("<input type=\"IMAGE\" width=\"$rimg_width\" ");  
...
echo("<input type=\"hidden\" name=\"from\" value=\"getverifyLP.php\" >\n");
echo("</form>");
echo("<form action=\"getverifyLP.php\" method=\"post\" name=\"gorefresh\">");
echo("<input type=\"hidden\" name=\"type\" value=\"$type\" >");
echo("<input type=\"hidden\" name=\"grn\" value=\"$grn\" >");
echo("<input type=\"hidden\" name=\"complete\" >");
echo("<input type=\"hidden\" name=\"order\" value=\"$order\">\n");
