...
function getProductInfo($Link) {
    $sql = 'SELECT PROD_PROFILE.PROD_ID,
            ...
            WHERE PROD_PROFILE.UOM = UOM.CODE
            ORDER BY PROD_ID';
    if (!($result = ibase_query($Link, $sql))) {
        ...
        if (isset($_POST['third_no'])) {
            if ($_POST['third_no'] !== '') {
                ...
                if (empty($errors)) {
                    ...
                    $tpl = file_get_contents('/var/www/html/whm/receive/ISSN-new.prn');
        ...
        if (isset($_POST['action_continue_x'])) {
            ...
            if (empty($errors)) {
                ...
                if (count($result) == 8 && $result[7] == 'Processed successfully') {
                    //-- it's foolish code, but I have no enough time for research
                    //-- of base to process it beautifully
                    $ssn_original = substr($result[0],0,4) . '0100';
                    //--
