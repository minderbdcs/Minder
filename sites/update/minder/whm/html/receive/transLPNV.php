...
//phpinfo();
/* 15/07/2013 had to take out the javascript disable of buttons to prodcomplete
   so use a session variable instead
if (isset($_SESSION['token'])) {
    if (isset($_POST['token'])) {
        if ($_POST['token'] != $_SESSION['token']) {
            $wkTransPONVProcess = "NOTOK";
            header("Location: getdelivery.php?message=Already+Being+Processed" );
            exit();
        } else {
            // process form
            // reset session token
        ...
        $_SESSION['token'] = md5( 'transLPNV.php'  . time() );
        $wkTransPONVProcess = "OK";
    }
}
if (isset($_SESSION['token'])) {
...
/* ================================================================================================ */
	...
	if (($useVehicleDesc1[0] == "T" ) and
	    ...
	    ($useVehicleDesc3[0] == "T" ))
	{
		$wk_error_screen = "verifyLPOtherQty.php";
	} else {
		$wk_error_screen = "verifyLP.php";
	}
...
function errorHandler2( $errno, $errstr, $errfile, $errline, $errcontext)
{
	...
	//$log = fopen('/tmp/transLPNV.log' , 'a');
	$log = fopen('/data/tmp/transLPNV.log' , 'a');
...
// reset pick lines for newly arrived product 

function doResetIssn( $wkProduct)
{
	global $Link, $dbTran;
	$my_message = "";
	/* write transaction */
	$Query = "SELECT CNT FROM EXECUTE PROCEDURE RESET_ISSN_ORDERS('AS TO OP HAVE STOCK'";
	{
		$Query .= ",'" . $wkProduct."')";
	}
...
// ============================================================================================================
	...
	// check my grn exists
	$Query = "select grn  from grn  where grn='" . $grn . "'"; 
	//echo($Query);
	if (!($Result = ibase_query($Link, $Query)))
	{
	...
	ibase_free_result($Result);
	if (is_null($wkGrnGrn )) {
		// probably tried to do re-verify but have lost the grn to work on
		// so have to create a new one
	}
...
*/
	...
	/* if must do product creation before the labels
           lookup  the products short desc
           if the product don't exist or the short_desc = prod_id
           then must go to ../query/product.php first
        */
	if ($useNewProduct[0] == "F" )
	{
		//$wkProductDesc  = getProductDescription($Link,  $product );
		$wkProductDesc  = getProductDescription($Link,  $product, $owner );
		if ($wkProductDesc[0] == "" or $wkProductDesc[0] == $product)
		{
			if (isset($cancel_product))
			{
				//echo("got cancelproduct");
				//commit
				ibase_commit($dbTran);
				$dbTran = ibase_trans(IBASE_DEFAULT, $Link);
				header("Location: " . $wk_error_screen . "?noprod=T" );
				exit;
			} else {
				//echo("got no cancelproduct");
				//commit
				ibase_commit($dbTran);
				$dbTran = ibase_trans(IBASE_DEFAULT, $Link);
				header("Location: ../query/product.php?from="  . urlencode($_SERVER['PHP_SELF']));
				exit;
			}
		}
	}
		...
		if ($owner <> "")
		{
			...
			if ($my_responsemessage <> "Processed successfully ")
			{
				if ($my_message <> "")
				{
					...
					//header("Location: verifyLP.php?" . $my_message );
					header("Location: " . $wk_error_screen . "?" . $my_message );
					exit();
				}
			}
		}
...
*/
		...
		if ($my_result[sizeof($my_result) - 1] <> "Processed successfully ")
		{
			...
			//header("Location: " . $wk_error_screen . "?" . $my_message . urlencode($wk_db_error_msg));
			exit();
		}
	}
	...
	// want to  update orders to OP that were AL status for this product
	// use the change reset_issn db procedure
	...
	else
	{
                ...
                if ($wk_receive_weights == "F")
		{
			    ...
			    ($useVehicleDesc3[0] == "T" ))
			{
				...
				header("Location: verifyLPOtherQty.php?message=" . urlencode("Processed OK") );
			} else {
