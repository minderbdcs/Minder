...
$Query = "select person_id, first_name, last_name from person where person_type in ('LE','RP','CO','CS','CV','CR','IS')  order by first_name, last_name "; 
$Query = "select person_id, first_name, last_name from person where person_type in ('LE','RP','CO','CS','CV','CR','IS') and status = 'CU'  order by first_name, last_name "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
...
<?php
{
	echo("document.getssnqtys.message.value=\"Select Sent From\";\n");
	echo("document.getssnqtys.retfrom.focus();\n");
}
?>
</script>
</html>
