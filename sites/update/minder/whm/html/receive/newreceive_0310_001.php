...
function getVarietyList($Link, $grnNo)
{
    ...
    $query  = "SELECT CODE, DESCRIPTION
               ...
               WHERE SSN_TYPE =
                     (SELECT SSN_TYPE
                      FROM SSN
                      WHERE SUBSTRING(SSN_ID FROM 1 FOR 4) LIKE ?)";
    $qp     = ibase_prepare($Link, $query);
    $result = ibase_execute($qp, $grnNo);
    //$result = ibase_query($Link, $query);
    if (false !== $result) {
        while (false !== ($row = ibase_fetch_row($result))) {
            $output[$row[0]] = $row[1];
            ...
            if ($_POST['third_no'] !== '') {
                ...
                if (empty($errors)) {
                    ...
                    //-- proceed while not printed all labels in 2nd set
                        if ($result[4] > $rQty2) {
                            include 'yy_0310_001.php';
                            $NIOB_res = sendNIOBTransaction($Link,
                                                            $cIssn2);
                                $NIBC_res = sendNIBCTransaction($Link,
                                                                    $cIssn2);
                                    $UIO_res = sendUIO1Transaction($Link,
                                                        $cIssn1,
                                                        $_POST['third_no']);
