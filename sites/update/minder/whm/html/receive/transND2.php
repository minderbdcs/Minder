...
//phpinfo();
	...
	*/
	...
	elseif (($Row = ibase_fetch_row($Result)))
	{
		$tran_response =  $Row[0] . ':::::::';
		$log = fopen('/data/tmp/transnd2.log' , 'a');
		fwrite($log,"able to add transaction\n");
		fwrite($log,"$tran_response\n");
		fclose($log);
	...
	if (isset ($tran_message))
	{
		if ($tran_message <> "Processed successfully")
		{
			...
			$log = fopen('/data/tmp/transnd2.log' , 'a');
			fwrite($log,"response not ok to add transaction\n");
			fwrite($log,"$tran_message\n");
			fclose($log);
	...
	{
		$log = fopen('/data/tmp/transnd2.log' , 'a');
		fwrite($log,"owner empty go back to getdelivery2\n");
		fclose($log);
