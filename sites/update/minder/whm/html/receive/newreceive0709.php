...
function showPageVerify($Link, $grnNo, $orderNo, $errors = array()) {
    ...
    $varietyList = getVarietyList($Link);
    $brandList =  getBrandList($Link);

    include 'nr_verify_p1.php';
}
        ...
        if (isset($_POST['third_no'])) {
            if ($_POST['third_no'] !== '') {
                ...
                if (empty($errors)) {
                    ...
                    if ($result[1] > $rQty1) {
                        include 'yy.php';
                        $trans_result = sendNIOBTransaction($Link,
                                                            $cIssn1);
/*                        if (count($trans_result) == 8 &&
                            $trans_result[7] == 'Processed successfully') {*/
                            $trans_result = sendNIBCTransaction($Link,
                                                                $cIssn1);
                            if (count($trans_result) == 8 &&
                                $trans_result[7] == 'Processed successfully') {
                                sendUIO1Transaction($Link,
                                                    $cIssn1,
                                                    $_POST['third_no']);
                                    $rQty1++;
                                    $save = fopen('/asset.rf/' . $_POST['printer_id'] .
                                ...
                                if (count($trans_result) == 8) {
                                    $errors['unknown'] = $trans_result[7];
                                } else {
                                    $errors['unknown'] = $trans_result[0];
                                }
                                showPageVerify($Link,
                                               $_POST['real_grn_no'],
                                               $_POST['real_order_no'],
...
/*                        } else {
                        ...
                        if ($result[4] > $rQty2) {
                            include 'yy.php';
// code below may be uncommented if properly tested on DB
/*                            $trans_result = sendNIOBTransaction($Link,
                                                            $cIssn2);
                            if (count($trans_result) == 8 &&
                                $trans_result[7] == 'Processed successfully') {
                                ...
                                if (count($trans_result) == 8 &&
                                    $trans_result[7] == 'Processed successfully') {
                                    $trans_result = sendUIO1Transaction($Link,
                                                                        $cIssn2,
                                                                        $_POST['third_no']);
                                    if (count($trans_result) == 8 &&
                                        $trans_result[7] == 'Processed successfully') {*/
