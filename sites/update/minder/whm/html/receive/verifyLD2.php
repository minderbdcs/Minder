...
function processCustomer() {
	var dowhat;
	document.getcustomer.retfrom.value = document.getssnqtys.retfrom.value ;
	document.getcustomer.owner.value = document.getssnqtys.owner.value ;
	document.getcustomer.received_ssn_qty.value = document.getssnqtys.received_ssn_qty.value ;
	document.getcustomer.location.value = document.getssnqtys.location.value ;
	document.getcustomer.printer.value = document.getssnqtys.printer.value ;
	document.getcustomer.problem.value = document.getssnqtys.problem.value ;
	document.getcustomer.submit();
  return true;
}
...
if (!isset($grnorder))
{
    // use last saved grnorder
	$grnorder = getBDCScookie($Link, $tran_device, "grnorder" );
	if ($grnorder == "")
	{
		unset($grnorder);
	}
...
if (isset($grnorder))
{
	...
	while ( ($Row = ibase_fetch_row($Result)) ) {
		...
		setBDCScookie($Link, $tran_device, "grnorder", $grnorder);
/*
...
// if the default wh_id  has no RC locations use the current wh id
// check there are RC locations
$Query = "select first 1  ln.wh_id, ln.locn_id, ln.locn_name from location ln  where ln.wh_id = '" . $default_wh_id . "' and ln.store_area = 'RC'  " ; 
if (!($Result = ibase_query($Link, $Query)))
{
...
if (trim($wk_test_wh == ""))
{
	$default_wh_id = $wk_current_wh_id ;
	setBDCScookie($Link, $tran_device, "WH_ID", $default_wh_id);
	$wk_log_msg = "default_wh_id has no RC Locations changed to use Current WH :" . $default_wh_id;
	//writeLogMsgs($wk_log_msg);
}
...
echo("Received :<input type=\"number\" name=\"received_ssn_qty\" size=\"4\" maxlength=\"4\" min=\"1\" max=\"550\" value=\"$received_ssn_qty\" class=\"default\"> Items<br>\n");
...
//$Query = "select device_id from sys_equip where device_type = 'PR' ";
//$Query .= "  and  (sys_equip.wh_id = '" . $default_wh_id . "'" ; 
//$Query .= "  or    sys_equip.wh_id is null) " ; 
//$Query .= " order by device_id "; 
//$Query = "select device_id from sys_equip join options o1 on o1.code in ('ISSN','ISSN_PRODUCT') and o1.group_code = sys_equip.device_id || '_FORMAT' where sys_equip.device_type='PR'";
//$Query .= "  and  sys_equip.wh_id = '" . $default_wh_id . "'" ; 
//$Query .= " and sys_equip.equipment_status = 'OK'" ; 
//$Query .= " group by sys_equip.socket_pause, device_id  "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
...
*/
{
	...
	echo("<input type=\"hidden\" name=\"from\" value=\"verifyLD2.php\" >\n");
	echo("</form>");
	echo ("</td>");
	echo ("<td>");
	$alt = "More";
	echo("<form action=\"customerplus.php\"  method=\"post\" onsubmit=\"return processCustomer();\" name=getcustomer>\n");
	echo("<input type=\"IMAGE\" ");  
	echo(" width=\"$rimg_width\" ");  
	echo('SRC="/icons/whm/button.php?text=' . $alt . '&fromimage=');
	echo('Blank_Button_50x100.gif" alt="' . $alt . '">');
