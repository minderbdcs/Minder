...
{	
	...
	$Query2 .= "from pick_item pi left outer join pick_order po on po.pick_order = pi.pick_order left outer join sys_equip se on se.device_id = pi.device_id where pi.pick_line_status in ('AL','PG','PL') " ;
...
echo ("</TABLE>\n");
echo ("<TABLE BORDER=\"1\">\n");
echo ("<TR><TH>Waiting Products</TH></TR>\n");
echo ("<TR><TH>#</TH>\n");
echo ("<TH>Dev</TH>\n");
echo ("<TH>Cmp</TH>\n");
echo ("<TH>Ctry</TH>\n");
...
echo ("<TH>Prod</TH></TR>\n");
{
	$Query2 = "SELECT count(distinct pi.prod_id),pi.device_id,po.company_id,po.p_country,se.last_person   ";
...
//	$Query2 .= " AND (NOT pi.prod_id IS NULL)";
	...
	while ( ($Row3 = ibase_fetch_row($Result2)) ) 
	{
		...
		echo ("<TD>$Row3[3]</TD>\n");
		echo ("<TD>$Row3[4]</TD>\n");
		$Query3 = "select first 1 prod_id from pick_item where device_id='" . $Row3[1] . "' and pick_line_status in ('AL','PG','PL') and (reason is null or reason = '' or reason ='NOT ENOUGH STOCK') order by pick_line_priority ";
