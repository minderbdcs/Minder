...
function processEdit() {
  if ( document.cancelorder.order.value=="")
  {
  	document.cancelorder.message.value="Must Enter the Order";
	document.cancelorder.order.focus()
  	return false
  }
  return true;
}
...
if ($wk_order_id <>"") 
{
	...
	$location = $wk_from_device . 'F|' . $wk_printer;
	...
	// then cancel it
	$my_ref =  $wk_order_id . '|' ;
	$transaction_type = "PKCO";
	$tran_tranclass = "K";
	$my_object = $wk_order_id;
	$location = "          ";
	$my_sublocn = "";
	$myref = "Set for Cancel";
	$tran_qty = 0;
	$my_source = "SSSSSSSSS";
	...
	// what about the pick_location table for this order ?
	// and items picked to conveyor
}
...
if ($wk_order_id == "")
{
	...
	echo("document.cancelorder.order.focus();\n");
	if ($wk_message <> "")
	{
		echo("document.cancelorder.message.value=\"" . $wk_message . " Enter Order to Cancel\";\n");
	}
	else
	{
		echo("document.cancelorder.message.value=\"Enter Order to Cancel\";\n");
	}
	echo("</script>");
}
else
{
	whm2buttons('Cancel', 'cancel.php',"Y","Back_50x100.gif","Back","accept.gif");
	echo("<script type=\"text/javascript\">");
	echo("document.cancelorder.order.focus();\n");
	if ($wk_message <> "")
	{
