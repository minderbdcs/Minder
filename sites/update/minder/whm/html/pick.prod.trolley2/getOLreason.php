...
if ($ssn <> '')
{
	...
	echo("<INPUT type=\"text\" readonly name=\"uom\" size=\"2\" value=\"$uom\"><br>");
...
$Query = "select code, description from options where group_code='CAN_PICK'";
if (!($Result = ibase_query($Link, $Query)))
{
	echo("Unable to Read Options!<br>\n");
	exit();
}
//echo("<br>ShortFall Reason<select name=\"reference\" size=\"4\" onclick=\"this.form.submit()\" >");
//echo("<br>ShortFall Reason<select name=\"reference\" size=\"4\" onchange=\"this.form.submit()\" >");
echo("<br>ShortFall Reason<select name=\"reference\" size=\"4\"  >");
// Fetch the results from the database.
