...
/* # save my order */
  var mytype;
	/* alert("in saveMe"); */
	/* alert(myorder); */
  	document.getdetails.salesorder.value = myorder; 
...
echo("<FORM action=\"getordersissn.php\" method=\"post\" name=\"getdetails\">\n");
echo("<INPUT name=\"message\" type=\"text\" size=\"60\" readonly value=\"" );
if (isset($wk_message))
{
	echo($wk_message);
}
echo (" Choose Order to Pick ");
echo("\"><br>\n");
$got_ssn = 0;
if (!isset($pickuser))
{
...
echo("<INPUT name=\"pickuser\" type=\"hidden\" value=\"") ;
echo($pickuser) ;
echo("\">") ;
echo("<INPUT name=\"allocatedevice\" type=\"hidden\" value=\"") ;
echo($allocatedevice) ;
echo("\">") ;
echo("<INPUT name=\"allocateorders\" type=\"hidden\" value=\"1\" >\n") ;
echo("<INPUT name=\"salesorder\" type=\"hidden\" value=\"\" >\n") ;
...
$wk_allow_prod  = "" ;
$wk_allow_company  = "" ;
$Query = "select wh_id, prod_id,company_id from issn where ssn_id = '" . $scanned_ssn . "'"; 
if (!($Result = ibase_query($Link, $Query)))
{
