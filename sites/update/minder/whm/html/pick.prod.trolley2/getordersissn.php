...
function callRICO()
{
   ...
   ajaxEngine.registerRequest('myLocnsList', 'checkLocns3.php');
   ajaxEngine.registerAjaxElement('locnsempty3');
}
</script>
*/
?>
...
-*	alert(""); *-
	document.getdetails.location1.focus();
}
*/
</script>
</head>
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__);
...
function errorHandler2( $errno, $errstr, $errfile, $errline, $errcontext)
{
	...
	$log = fopen('/data/tmp/issn.orders.log' , 'a');
...
function logtime2( $Link,  $message)
{
	...
	$log = fopen('/data/tmp/issn.orders2.log' , 'a');
		$wk_current_time = "";
...
echo("<div id=\"col3\">");
echo("<FORM action=\"getordersissn.php\" method=\"post\" name=\"getdetails\" onsubmit=\"return processSubmit('0');\">\n");
$got_ssn = 0;
if (!isset($pickuser))
{
	$pickuser = $tran_user;
...
$wk_allow_company  = "" ;
$wk_allow_qty  = 0 ;
$wk_allow_original_ssn  = "" ;
$Query = "select wh_id, locn_id, prod_id,company_id, current_qty,original_ssn from issn where ssn_id = '" . $scanned_ssn . "'"; 
if (!($Result = ibase_query($Link, $Query)))
{
...
else
{
	if ( ($Row = ibase_fetch_row($Result)) ) {
		...
		$wk_allow_qty  = $Row[4] ;
		$wk_allow_original_ssn  = $Row[5] ;
	}
}
//release memory
ibase_free_result($Result);

// check issn pick type
$wk_pick_allowed_issn_type  = "P";

if ($wk_allow_company  !== "" ) {
	$Query = "select options.description  from options where options.group_code='PICK' and options.code = 'ISSN|" . $wk_allow_company . "'  "; 
	if (!($Result = ibase_query($Link, $Query)))
	{
	...
	{
		if ( ($Row = ibase_fetch_row($Result)) ) {
			$wk_pick_allowed_issn_type  = $Row[0] ;
		}
	}
	//release memory
	ibase_free_result($Result);
...
if ($wk_dopkal == "T") {
	$Query = "select first " . $allocateorders . " distinct p2.pick_order , p2.volume, p2.volume_uom ,p2.other1,  "; 
	$Query .= " case when (p1.prod_id is null) then p1.ssn_id else p1.prod_id end as Part, "; 
	$Query .= " case when (p1.prod_id is null) then (select s1.ssn_description from ssn s1 where s1.ssn_id = '" . $wk_allow_original_ssn . "')  else (select p3.short_desc from prod_profile p3 where p3.prod_id = p1.prod_id) end as PartDesc "; 
	$Query .= "from pick_item p1 ";
	$Query .= "join pick_order p2 on p1.pick_order = p2.pick_order ";
	$Query .= "where p1.pick_line_status in ('OP','UP') ";
	//if do picking by product
	if ($wk_pick_allowed_issn_type  == "P") {
		$Query .= "and    p1.prod_id = '" . $wk_allow_prod . "'   ";
	} else {
		//if do picking by ssn
		if ($wk_pick_allowed_issn_type  == "S") {
			$Query .= "and   p1.ssn_id = '" . $wk_allow_original_ssn . "' ";
		} else {
			//do picking by issn
	...
	$Query = "select distinct p2.pick_order , p2.volume, p2.volume_uom ,p2.other1, "; 
	$Query .= " case when (p1.prod_id is null) then p1.ssn_id else p1.prod_id end as Part, "; 
	$Query .= " case when (p1.prod_id is null) then (select s1.ssn_description from ssn s1 where s1.ssn_id = '" . $wk_allow_original_ssn . "')  else (select p3.short_desc from prod_profile p3 where p3.prod_id = p1.prod_id) end as PartDesc "; 
	$Query .= "from pick_item p1 ";
	$Query .= "join pick_order p2 on p1.pick_order = p2.pick_order ";
...
/* if no orders found have a dud field for the location and produce an error message */
if ($wk_count == 0) {
	echo("<tr>");
	echo("<td>" . "<input name=\"location1\" type=\"text\" size=\"10\" id=\"location1\" onchange=\"return processEdit('1');\" >" . "</td>");
	echo("<td>" . "<input name=\"order1\" type=\"text\" readonly size=\"10\" id=\"order1\" value=\"None Left\">" . "</td>");
	echo("<td>" . "<input name=\"scancount1\" type=\"text\" readonly size=\"2\" value=\"1\">" . "</td>");
	echo("</tr>");
}
echo("</tbody>");
echo ("</table>\n");
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " transaction loop start");
	...
	$wk_loop = 1;
	$wk_db_error_count = 0;
	//while ( ($wk_loop <= count($allocateorder) ) or  ($wk_db_error_count > 3) ) :
	while ( ($wk_loop <= count($allocateorder) ) and ($wk_db_error_count <= 3) ) :
	{

		if ( $allocateorder[$wk_loop] == "None Left" )
		{
			$wk_isok = "None";
		}
		...
		if ($wk_dopkal == "T") {
			if ($wk_isok == "T") {
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKAL start");
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKAL stop ");
			}
		}
		...
		if ($wk_isok == "T") {
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKOL start ");
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKOL stop  ");
		}
		if ($wk_isok == "T") {
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKiL start ");
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " PKIL stop  ");
		}
		if ($wk_isok == "T") {
			...
			$Query4 .= "where p1.pick_line_status in ('AL','PG','PL') ";
		...
		if ($wk_isok == "T" and $wk_next_label == "") {
			$my_order  = $allocateorder[$wk_loop];
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " TRPK start ");
			doTRPK ($Link, $my_order );
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " TRPK stop  ");
		}
...
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . " Post Commit");
//close
//ibase_close($Link);
...
if (count($allocateorder) > 0 )
{
	...
	if ($wk_next_label_total == "") {
		...
		echo("<FORM action=\"getfromissn.php\" method=\"post\" name=\"continueordrs\" >\n");
	} else {
		echo("<FORM action=\"getfromlocn.php\" method=\"post\" name=\"continueordrs\" >\n");
		// need to continue with this order and scan another issn for this product
	}
...
else
{
	...
	/*
	{
		...
		echo("<INPUT type=\"hidden\" name=\"from\" value=\"getordersissn.php\">");
		echo("</form>");
		echo ("</td>");
	}
	*/
...
//echo("<div  id=\"locnsempty3\"></div>\n");
//echo("<div  id=\"picked\"></div>\n");
logtime2( $Link,  __LINE__ . ":" . __FUNCTION__ . "============================== end of screen ==============================" );
?>
</body>
</html>
