...
if (isset($_COOKIE['LoginUser']))
{
	...
	//check whether we do a PKUA or Not
	$Query = "select max_pick_lines from control";
	if (!($Result = ibase_query($Link, $Query)))
	{
		header("Location: pick_Menu.php?message=Unable+to+Read+Total+from+Control!");
		exit();
	}
	...
	if (!($Result = ibase_query($Link, $Query)))
	{
		header("Location: pick_Menu.php?message=Unable+to+Read+Total+on+Device!");
		exit();
	}
	if ( ($Row = ibase_fetch_row($Result)) ) {
		$wk_pick_cnt = $Row[0];
	...
	*/
	...
	} /* end of process for pkua */
	else
	{
		// no pkua since too many lines to update
		//commit
		ibase_commit($dbTran);
		//close
		//ibase_close($Link);
