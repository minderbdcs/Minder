...
*/
{
	...
	//$Query .= " order by (select wk_seq from pick_item_prod_seq(p3.wh_id,p3.company_id,p1.prod_id,p1.ssn_id)), (select * from nonullp(p1.prod_id)) || (select * from nonullp(s1.prod_id) ) ";
	$Query .= " order by  p1.pick_line_priority, ";
	$Query .= " (select wk_seq from pick_item_prod_seq(p3.wh_id,p3.company_id,p1.prod_id,p1.ssn_id)), (select * from nonullp(p1.prod_id)) || (select * from nonullp(s1.prod_id) ) ";
...
// now get the products size grouping
if (isset($prod_no))
{
	$Query = "select size_type  "; 
	$Query .= "from prod_profile p1  ";
	$Query .= " where p1.prod_id = '".$prod_no."'";
	$wk_prod_size = "";
	
	if (!($Result = ibase_query($Link, $Query)))
	{
		echo("Unable to query prod_profile!<BR>\n");
		exit();
	}
	// Fetch the results from the database.
	if (($Row = ibase_fetch_row($Result)) )
...
else
{
	...
	{
		$Query .= " where s3.prod_id in (select prod_id from prod_profile where size_type = '".$wk_prod_size."') ";
	}
	// wh must match wh of order
	$Query .= " and s3.wh_id in (" . $Query2 . ") ";
	// company must match company of order
