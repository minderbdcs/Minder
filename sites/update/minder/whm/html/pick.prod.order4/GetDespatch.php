...
{
	$my_ref = $wk_printer . $wk_order_id . "|" .  $wk_order_id . '|' ;

	$Query = "EXECUTE PROCEDURE PC_LABEL_DESPATCH '" . $wk_order_id . "','" . $wk_printer . "'";
...
if ($wk_printer == "")
{
	$Query = "select EXPORT_DESPATCH_PRINTER FROM CONTROL ";
	//echo($Query);
	if (!($Result = ibase_query($Link, $Query)))
	{
		echo("Unable to Read Control");
		exit();
	}
	$wk_printer = "PF";
...
echo (" <FORM action=\"GetDespatch.php\" method=\"post\" name=printdespatch>");
echo (" <P>");
