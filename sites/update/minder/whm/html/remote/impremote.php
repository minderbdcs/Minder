...
// connect to db
   ...
   $Query = "select description  from options where group_code = 'IMPREMOTE' ORDER BY CODE "; 
   //echo($Query);
   if (!($Result = ibase_query($Link, $Query)))
   {
...
//var_dump($wkResults);
        $contentarea = "  <tr><td colspan=5>
          <textarea name=\"content\" cols=\"80\" rows=\"9\">". print_r($content,True)."</textarea><br>
    ...
    <table align=center ><tr><td>
      <form action="$docself" method=get >
        <fieldset width=500px>
          ...
          <tr>
          <td align=center><input type="submit" name="action" value="Save"></td><td></td><td align=center><input type=button value="Help" onclick="togglevisibility('help');"></td><td></td><td align=center><input type=button value="Back" onclick="location.href='../..';"></td></tr>
          </table>
        </fieldset>
      </form>
    </td></tr>
    ...
    <div id="help">
      ...
      <h2>Import Transactions from Remote</h2>

      Version: $ver<br>
      By: Frank Leih  - MinderSeries.com<br>
      <br>
      Choose the Device to Import From then click Save to Start the Import.<br>
      <br>
    </div>
    </td></tr>    
    </table>
