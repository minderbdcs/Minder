...
function getDeviceInfo($Link, $deviceId)
{
	...
	$Query = "SELECT se_brand, se_model,default_pr_printer, ip_address from sys_equip where device_id = '".$deviceId."'";
	if (!($Result = ibase_query($Link, $Query)))
	{
	...
	return array($DBBrand, $DBModel , $DBIPAddress);
}
/* end of function */
...
if (($UserName == $DBLoginUser) && ($UserPassword == $DBUserPassword)) {
	...
	else
	{
		...
		setcookie("BDCSDATA","",time()+1864000,"/");
		if ($wkMyBW == "NETFRONT") {
			setcookie("LoginUser","$UserName|$DBDevice",time()+864000,"/");
			setcookie("LoginUser","$UserName|$DBDevice",time()+1864000,"/");
		} else {
		...
		$DBPrinter = "";
		$DBIPAddress = "";
		list ($DBBrand, $DBModel, $DBIPAddress) = getDeviceInfo($Link, $DBDevice);
		...
		if ($DBIPAddress == "DHCP")
		{
		    if (!empty($DBWH_ID)) 
		    {
                    	$query = "UPDATE SYS_EQUIP SET WH_ID = '" . $DBWH_ID . "' WHERE DEVICE_ID = '" . $DBDevice . "'";
                    	$result = ibase_query($Link, $query);
                    	if (false === $result) {
                        	echo "Unable to Update SYS_EQUIP!\n";
                    	}
                    }
		...
		$Query = "UPDATE OR INSERT into session(device_id, code, description, create_date) values('" . $DBDevice . "','TZ','" . $TZ . "','NOW')";
                $result = ibase_query($Link, $Query);
                if (false === $result) {
                	echo "Unable to Update Session!\n";
                }
		$Query = "UPDATE OR INSERT into session(device_id, code, description, create_date) values('" . $DBDevice . "','DBTZ','UTC','NOW')";
                $result = ibase_query($Link, $Query);
                if (false === $result) {
                	echo "Unable to Update Session!\n";
                }
		...
		$_SESSION['TZOFFSET'] = (string)$wktimeOffset ;
