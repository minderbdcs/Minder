...
*/
  if (document.getlocn.required_qty.value > 1)
  {
    if (document.getlocn.qtypicked.value == "")
    {
      /* must enter qty picked */
      document.getlocn.message.value = "Enter the Qty Picked";
      document.getlocn.qtypicked.focus();
      return false;
    }
    if ( chkNumeric(document.getlocn.qtypicked.value)==false)
    {
  	document.getlocn.message.value="Qty Picked Not Numeric";
	document.getlocn.qtypicked.focus();
  	return false;
    }
    if (( document.getlocn.qtypicked.value + 0) > (document.getlocn.required_qty.value + 0))
    {
  	document.getlocn.message.value="Cannot Pick More than Required Qty";
	document.getlocn.qtypicked.focus();
  	return false;
    }
  }
...
*/
	...
	$Query .= " where s3.prod_id = '".$prod_no."'";
	$Query .= " and s3.current_qty > 0 ";
	$Query .= " and (s3.wh_id = control.default_wh_id) ";
	$Query .= " and (s3.locn_id starting 'PM') ";
	$Query .= " and pos('" . $allowed_status . "',s3.issn_status,0,1) > -1";
	$Query .= " group by s3.wh_id, s3.locn_id";
}
...
<?php
{
	echo("document.getlocn.message.value = \"Scan Location\";\n");
	if ($prod_no == "NOPROD")
	{
