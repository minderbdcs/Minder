...
session_start();
{
	...
	if (isset($ssn_to))
	{
		if ($ssn_to <> "")
		{
			$field_type = checkForTypein($ssn_to, 'BARCODE','SSN' ); 
			if ($field_type == "none")
			{
				// not an ssn - perhaps a location
				$field_type = checkForTypein($ssn_to, 'LOCATION' ); 
				if ($field_type == "none")
				{
					header("Location: gettossn.php?location=" . urlencode($from_location)."&message=" . urlencode("Not an SSN or Location"));
				}
				else
				{
					$ssn_data = substr($ssn_to, $startposn);
					$ssn_to = $ssn_data;
...
*/
	...
	if (strlen($to_location) > 10)
	{
		header("Location: gettossn.php?location=".$to_location."&message=Invalid+Location!");
		exit();
	}
	// check putaway id
	$wk_putaway = "";
...
/*
	...
	{
		header("Location: gettossn.php?location=".$to_location."&message=Can+t+read+SESSION!");
		exit();
	}
	else
...
*/
	...
	if ($my_responsemessage <> "Processed successfully ")
	{
		...
		header("Location: gettolocn.php?location=".$to_location."&".$my_message);
                exit();
	}
