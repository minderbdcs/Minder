...
$Query .= "left outer join issn s3 on s3.prod_id = p1.prod_id and s3.current_qty > 0 ";
...
$Query = "select first 1 p1.ssn_id, p1.pick_label_no, p1.prod_id, s2.ssn_type, p2.short_desc, p2.uom, p1.pick_order_qty, p1.picked_qty, p1.pick_order "; 
$Query .= "from pick_item p1 ";
$Query .= "left outer join pick_order p3 on p1.pick_order = p3.pick_order ";
$Query .= "left outer join issn s1 on p1.ssn_id = s1.ssn_id ";
$Query .= "left outer join ssn s2 on  s1.original_ssn = s2.ssn_id ";
$Query .= "left outer join issn s3 on p1.prod_id = s3.prod_id and s3.current_qty > 0 ";
$Query .= "left outer join prod_profile p2 on p1.prod_id = p2.prod_id ";
$Query .= "left outer join location l1 on  substr(p1.pick_location,1,2) = l1.wh_id ";
$Query .= " and   substr(p1.pick_location,3,10) = l1.locn_id ";
$Query .= " where ((p1.pick_line_status in ('AL','PG')) " ;
$Query .= " or (p1.pick_line_status ='PL' AND p1.picked_qty < p1.pick_order_qty) " ;
$Query .= " or (p1.pick_line_status ='PL' AND exists(select p4.pick_detail_id from pick_item_detail p4 where p4.pick_label_no = p1.pick_label_no and p4.despatch_location is NULL))) " ;
$Query .= " and p1.device_id = '".$tran_device."'";
...
//ibase_close($Link);
	...
	if (($got_ssn == 0) or ($prod_no == "NOPROD"))
	{
		...
		//echo('SRC="/icons/whm/pick.despatch.trolley.gif" alt="' . $alt . '"></INPUT>');
		echo("</FORM>");
		echo ("</TD>");
		//if ($wk_buttons == 2)
		if ($wk_buttons == 4)
