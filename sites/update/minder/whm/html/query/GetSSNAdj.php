...
<?php
	...
	{
		header("Location: query.php?message=Can+t+connect+to+DATABASE!");
		exit();
	}
...
function checkadjust($confirmpw )
{
	...
	if (isset($_POST['from'])) 
	{
		$wk_from_screen = $_POST['from'];
	}
	if (isset($_GET['from'])) 
	{
		$wk_from_screen = $_GET['from'];
	}
	if (!isset($wk_from_screen))
	{
		$wk_from_screen = "1ssn.php";
	}
	if (isset($_POST['ssn_id'])) 
	{
		$wk_from_issn = $_POST['ssn_id'];
	}
	if (isset($_GET['ssn_id'])) 
	{
		$wk_from_issn = $_GET['ssn_id'];
	}
	if (isset($ssn))
	{
		// check that ssn exists
...
echo("<input type=\"hidden\" name=\"from\" value=\"" . $wk_from_screen . "\" >");
echo("<input type=\"hidden\" name=\"ssn_id\" value=\"" . $wk_from_issn . "\" >");
...

{
	...
	if (isset($ssn))
	{
		whm2buttons('Adjust', $wk_from_screen,"Y","Back_50x100.gif","Back","adjustssn.gif");
	}
	else
	{
		whm2buttons('Accept', $wk_from_screen ,"Y","Back_50x100.gif","Back","accept.gif");
	}
}
//commit
ibase_commit($dbTran);
