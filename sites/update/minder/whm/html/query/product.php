...
//echo ("<div id=\"col2\">\n");
if (isset($from))
	echo("<form action=\"transPP.php\" method=\"post\" name=\"alterproduct\" onsubmit=\"return processEdit();\">\n");
else
	echo("<form action=\"prodlocn.php\" method=\"post\" name=\"alterproduct\">\n");
...
echo("Issue:");
echo("</td><td>");
echo("<input type=\"text\" name=\"issue\" value=\"$issue\" size=\"4\" maxlength=\"6\">");
echo("</td><td>");
...
echo("Net Wt:");
echo("</td><td>");
echo("<input type=\"text\" name=\"net_weight\" value=\"$net_weight\" size=\"4\" maxlength=\"6\">");
echo("</td><td>");
//echo("<input type=\"text\" name=\"net_weight_uom\" value=\"$net_weight_uom\" size=\"2\" maxlength=\"2\">");
//echo("<select name=\"net_weight_uom\" >\n");
echo("<select name=\"net_weight_uom\" class=\"sel3\" >\n");
$Query = "select code, description from uom where uom_type = 'WT' order by description "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
	echo("Unable to Read Net_Weight_UOM!<BR>\n");
	exit;
}
while ( ($Row = ibase_fetch_row($Result)) ) {
	if ($Row[0] == $net_weight_uom)
	{
		echo( "<OPTION value=\"$Row[0]\" selected >$Row[1]\n");
	}
	else
...
echo("Inner:");
echo("</td><td>");
echo("<input type=\"text\" name=\"inner_qty\" value=\"$inner_qty\" size=\"4\" maxlength=\"6\" onchange=\"recalcInner('QTY');\">");
echo("</td><td>");
//echo("<input type=\"text\" name=\"inner_uom\" value=\"$inner_uom\" size=\"2\" maxlength=\"2\">");
//echo("<select name=\"inner_uom\" >\n");
echo("<select name=\"inner_uom\" class=\"sel3\" >\n");
$Query = "select code, description from uom where uom_type = 'UT' order by description "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
	echo("Unable to Read Inner_UOM!<BR>\n");
	exit;
}
while ( ($Row = ibase_fetch_row($Result)) ) {
	if ($Row[0] == $inner_uom)
...
echo("</select>\n");
echo("</td><td>");
echo("<input type=\"text\" name=\"per_inner_qty\" value=\"$per_inner_qty\" size=\"4\" maxlength=\"6\" onchange=\"recalcInner('PER');\">");
echo("</td></tr>");
echo("<tr><td>");
echo("Pack:");
echo("</td><td>");
echo("<input type=\"text\" name=\"order_qty\" value=\"$order_qty\" size=\"4\" maxlength=\"6\">");
echo("</td><td>");
//echo("<input type=\"text\" name=\"order_uom\" value=\"$order_uom\" size=\"2\" maxlength=\"2\">");
//echo("<select name=\"order_uom\" >\n");
echo("<select name=\"order_uom\" class=\"sel3\" >\n");
$Query = "select code, description from uom where uom_type = 'UT' order by description "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
...
echo("Pack Wt:");
echo("</td><td>");
echo("<input type=\"text\" name=\"order_weight\" value=\"$order_weight\" size=\"4\" maxlength=\"6\">");
echo("</td><td>");
//echo("<input type=\"text\" name=\"order_uom\" value=\"$order_weight_uom\" size=\"2\" maxlength=\"2\">");
//echo("<select name=\"order_weight_uom\" >\n");
echo("<select name=\"order_weight_uom\" class=\"sel3\" >\n");
$Query = "select code, description from uom where uom_type = 'WT' order by description "; 
//echo($Query);
if (!($Result = ibase_query($Link, $Query)))
{
	echo("Unable to Read Order_Weight_UOM!<BR>\n");
	exit;
}
while ( ($Row = ibase_fetch_row($Result)) ) {
	if ($Row[0] == $order_weight_uom)
	{
		echo( "<OPTION value=\"$Row[0]\" selected >$Row[1]\n");
	}
	else
...
echo("<input type=\"text\" name=\"alternate\" value=\"$pp_alternate\" size=\"14\" maxlength=\"30\" >");
echo("</td>");
echo("</tr>");
