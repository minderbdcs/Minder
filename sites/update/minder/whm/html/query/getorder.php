...
select p1.pick_order 
 ...
 and    p2.pick_line_status in  ('PL','PG','AL','Al','Pg','Pl')
 and p3.pick_detail_status  in ('PL','PG','Pl','Pg','AL','Al')
 and p3.qty_picked > 0
...
if (sizeof($wk_orders_list) > 0)
{
	foreach ($wk_orders_list as $wk_key  => $wk_order)
	{
		...
		*/
		...
		/* dx queries */
		$Query7 = "SELECT po.pick_order,po.company_id, po.p_country, po.pick_order, po.pick_status,";
		$Query7 .= "po.over_sized, ";
		$Query7 .= " mer_day(po.create_date) || '/' || mer_month(po.create_date) || '/' || mer_year(po.create_date) || ' ' || mer_hour(po.create_date) || ':' || mer_minute(po.create_date) ";
		$Query7 .= " , po.address_label_date ";
		$Query7 .= " , mer_day(po.address_label_date) || '/' || mer_month(po.address_label_date) || '/' || mer_year(po.address_label_date) || ' ' || mer_hour(po.address_label_date) || ':' || mer_minute(po.address_label_date) ";
		$Query7 .= ",po.net_weight ";
		$Query7 .= ",po.volume, po.other1 ";
		$Query7 = "SELECT po.pick_order,po.company_id, po.p_country, po.pick_status,";
		$Query7 .= "po.over_sized, ";
		$Query7 .= " mer_day(po.create_date) || '/' || mer_month(po.create_date) || '/' || mer_year(po.create_date) || ' ' || mer_hour(po.create_date) || ':' || mer_minute(po.create_date) ";
		$Query7 .= " , po.address_label_date ";
		$Query7 .= " , mer_day(po.address_label_date) || '/' || mer_month(po.address_label_date) || '/' || mer_year(po.address_label_date) || ' ' || mer_hour(po.address_label_date) || ':' || mer_minute(po.address_label_date) ";
		$Query7 .= ",po.net_weight ";
		$Query7 .= ",po.volume, po.other1 ";
		$Query7 .= " from pick_order_dx po  ";
		//$Query7 .= " where po.pick_order = '" . $wk_order . "'";
		$Query7 .= " where po.pick_order LIKE  '%" . $wk_order . "%'";
	
		$Query8 = "SELECT  ";
		$Query8 .= "pd.despatch_status ";
		$Query8 .= ",pd.pack_type, ";
		$Query8 .= " mer_day(pd.create_date) || '/' || mer_month(pd.create_date) || '/' || mer_year(pd.create_date) || ' ' || mer_hour(pd.create_date) || ':' || mer_minute(pd.create_date) ";
		$Query8 .= " ,pd.pickd_exit, ";
		$Query8 .= " mer_day(pd.pickd_exit) || '/' || mer_month(pd.pickd_exit) || '/' || mer_year(pd.pickd_exit) || ' ' || mer_hour(pd.pickd_exit) || ':' || mer_minute(pd.pickd_exit) ";
		$Query8 .= ", pd.pickd_wt_actual";
		$Query8 .= " , pd.awb_consignment_no, pd.despatch_id ";
		$Query8 .= " , pd.pickd_carrier_id ";
		$Query8 .= " from pick_despatch pd ";
		//$Query8 .= " where pd.awb_consignment_no = '" . $wk_order . "'";
		$Query8 .= " where pd.despatch_id in (  "; 

		$Query9 = "SELECT pi.pick_label_no,  pi.prod_id, pi.pick_order_qty, pi.picked_qty, pi.pick_line_status, pi.over_sized,";
		$Query9 .= " mer_day(pi.create_date) || '/' || mer_month(pi.create_date) || '/' || mer_year(pi.create_date) || ' ' || mer_hour(pi.create_date) || ':' || mer_minute(pi.create_date) ";
		$Query9 .= ", pi.pick_pick_finish ";
		$Query9 .= ", mer_day(pi.pick_pick_finish) || '/' || mer_month(pi.pick_pick_finish) || '/' || mer_year(pi.pick_pick_finish) || ' ' || mer_hour(pi.pick_pick_finish) || ':' || mer_minute(pi.pick_pick_finish) ";
		$Query9 .= " ,pi.wh_id,pi.pick_location,pi.user_id,pi.ssn_id ";
		$Query9 .= " from pick_item_dx pi ";
		//$Query9 .= " where pi.pick_order = '" . $wk_order . "'";
		$Query9 .= " where pi.pick_order = " ;
	

		$Query10 = "SELECT  '',pid.ssn_id, '', pid.qty_picked, pid.pick_detail_status, '',";
		$Query10 .= " mer_day(pid.create_date) || '/' || mer_month(pid.create_date) || '/' || mer_year(pid.create_date) || ' ' || mer_hour(pid.create_date) || ':' || mer_minute(pid.create_date) ";
		$Query10 .= ", '' ";
		$Query10 .= " ,pid.from_wh_id,pid.from_locn_id,pid.user_id,pid.despatch_id ,pid.pick_detail_id ";
		$Query10 .= " from pick_item_detail_dx pid ";
		//$Query10 .= " where pid.pick_label_no = " ;
		$Query10 .= " where pid.pick_detail_status <> 'CN' and pid.ssn_id <> 'SSN' and pid.qty_picked > 0 and pid.pick_label_no = " ;

		$Query12 = "SELECT  '','' , '', issn.current_qty, issn.issn_status , '',";
		$Query12 .= " mer_day(issn.create_date) || '/' || mer_month(issn.create_date) || '/' || mer_year(issn.create_date) || ' ' || mer_hour(issn.create_date) || ':' || mer_minute(issn.create_date) ";
		$Query12 .= ", '' ";
		$Query12 .= " ,issn.wh_id,issn.locn_id,issn.user_id,'' ,'' ";
		$Query12 .= " from issn_dx ";
		$Query12 .= " where  issn.ssn_id  = " ;

		$Query13 = " SELECT pid.despatch_id from pick_item_detail_dx pid where pid.pick_order in  "; 
		$wkOrderNo = '';
		//echo ("<table border=\"1\">\n");
		// do query 1 order
		{
		...
		if ($wk_have_data == 0) {
			// have none so try the dx table
			$wk_query = $Query8;
			$wk_query = $Query8 . "('" . $wkOrderNo . "') group by pid.despatch_id)";
			$wk_query = $Query13 . "('" . $wkOrderNo . "') and (pid.despatch_id is not null) group by pid.despatch_id";
			...
			ibase_free_result($Result2);
			// remove last comma
			$wk_despatch_list = substr($wk_despatch_list, 0, -1);
			//echo $wk_despatch_list;

			if ($wk_despatch_list != "") 
			{
				$wk_query = $Query8 . "('" . $wk_despatch_list . "') )";
				//echo($wk_query);
				if (!($Result2 = ibase_query($Link, $wk_query)))
				{
					echo("Unable to Read Orders!<BR>\n");
...
echo("<table BORDER=\"0\" ALIGN=\"LEFT\">");
	...
	echo ("<td>");
{
	whm2buttons('GetOrder', 'query.php', "Y","Back_50x100.gif","Back","nextorder.gif");
}
//echo "<script>\n";
