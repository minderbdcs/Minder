...
//$wk_query1 = "select person_id,mer_hour(trn_date),count(distinct object) from transactions_archive where trn_type='TROL' and trn_code='P' and reference starting 'Replenish' and trn_date >'TODAY' group by person_id,mer_hour(trn_date) ";  
$wk_query1 = "select person_id,mer_hour(trn_date),count(distinct object) from transactions_archive where trn_type='TROL' and trn_code='P' and reference starting 'Replenish' and trn_date >= cast('$work_mm/$work_dd/$work_yy' as timestamp) and trn_date <= cast('$work_mm/$work_dd/$work_yy 23:59:59' as timestamp) group by person_id,mer_hour(trn_date) ";  
//echo $wk_query1;
{
	echo("<html>\n");
	echo("<head>\n");
	echo("<title>Replenished Products</title>\n");
	echo("<script language=\"JavaScript\" src=\"../includes/ts_picker.js\">\n");
	echo("</script>\n");
	echo("</head>\n");
	echo("<body>\n");
	echo("<h2>Replenish Totals per Day</h2>\n");
...

{
	...
	//echo '<form action="replenish_Menu.php" method="post" name="Export"> ';
