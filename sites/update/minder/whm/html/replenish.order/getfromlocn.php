...
*/
{
	$Query = "select first 1 p1.trn_line_no, p1.prod_id, p1.company_id, p2.short_desc, p2.uom, p1.to_wh_id  "; 
	$Query .= "from transfer_request p1 ";
	$Query .= "left outer join prod_profile p2 on p1.prod_id = p2.prod_id and p1.company_id = p2.company_id ";
	//$Query .= "left outer join location l1 on l1.wh_id = p1.to_wh_id and l1.locn_id = p1.to_locn_id ";
	$Query .= "left outer join issn i1 on p1.prod_id = i1.prod_id and p1.company_id = i1.company_id and p1.to_wh_id = i1.wh_id ";
	$Query .= "left outer join location l1 on i1.wh_id = l1.wh_id and i1.locn_id = l1.locn_id ";
...
// want all the required qtys for this prod
{
	...
	$Query .= "from transfer_request p1 ";
	$Query .= "left outer join location l1 on l1.wh_id = p1.to_wh_id  ";
	$Query .= " where p1.device_id = '".$tran_device."'";
	$Query .= " and p1.prod_id = '" . $prod_no . "' " ;
	$Query .= " and p1.company_id = '" . $company_id . "' " ;
	$Query .= " and p1.to_wh_id = '" . $wk_to_wh_id . "' " ;
	$Query .= " and p1.trn_status in ('AL','PG') " ;
	$Query .= " and p1.qty > 0 " ;
	//echo($Query);
...
// want ssn label desc
{
	...
	$Query .= " and  s3.company_id = '".$company_id ."'";
...
if ($ssn == '')
{
	...
	echo("<INPUT type=\"hidden\" name=\"to_wh_id\" value=\"$wk_to_wh_id\" >");
}
