...
if (isset($_COOKIE['LoginUser']))
{
	...
	// check the location entered
	$wk_found = 0;
	$Query = "SELECT 1 FROM LOCATION WHERE ";
	$Query .= " WH_ID = '" . substr($location,0,2)."' ";
	$Query .= " AND LOCN_ID = '" . substr($location,2,strlen($location) - 2)."'";	
	//print($Query); 
	if (!($Result = ibase_query($Link, $Query)))
	{
		header("Location: gettolocn.php?message=Unable+to+Read+Location!");
		exit();
		// processed ok
	}
	else
	...
	if ($wk_found == 0)
	{
		header("Location: gettolocn.php?message=Location+Not+Found!");
		exit();
	}
