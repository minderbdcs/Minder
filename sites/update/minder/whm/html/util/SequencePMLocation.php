...
function processQty() {
  ...
  if ( chkNumeric(document.showstock.startseq.value)==false)
  {
  	document.showstock.message.value="Starting Sequence Not Numeric";
	document.showstock.startseq.focus();
  	return false;
  }
...
*/

	{
		...
		$Query .= " order by substr(locn_id,1,2),substr(locn_id,3,4),substr(locn_id,7,8),substr(locn_id,5,6)";
		//echo $Query;
		...
		while ( ($Row = ibase_fetch_row($Result)) ) 
		{
			$wk_locn = $Row[0];
			$wk_cnt = $wk_cnt + 1;
			$Query2 = "update location set locn_seq = '" . $my_seq . "' where wh_id = '" . $wh . "' and locn_id = '" . $wk_locn . "'";
...
//else
{
	...
	echo("</td>");
	echo("<td colspan=\"2\">");
	echo ("<INPUT type=\"text\" name=\"startseq\"  ");
	echo(" size=\"10\" value=\"$startseq\" >\n");
	echo("</td>");
	echo("</tr>");
	whm2buttons('Accept', 'util_Menu.php',"Y","Back_50x100.gif","Back","accept.gif");
	echo("<script type=\"text/javascript\">document.showstock.wh.focus();</script>");
}
?>
</body>
</html>
