...
echo "<html>";
echo "<head>";
echo "<title>Allocate Orders</title>";
...
echo ("<table BORDER=\"0\">\n");
echo ("<tr><td>\n");
//echo("<br><br>Allocate to User:<SELECT name=\"pickuser\" >\n");
echo("<br><label for=\"pickuser\">Allocate to User:</label>\n");
echo("<select name=\"pickuser\" id=\"pickuser\" >\n");
...
echo("<label for=\"allocatedevice\">Allocate to Device:</label>\n");
echo("<select name=\"allocatedevice\" id=\"allocatedevice\" >\n");
// Fetch the results from the database.
while (($Row2 = ibase_fetch_row($Result))) {
...
echo("<label for=\"pickdevice\">Pick to Device:</label>\n");
echo("<select name=\"pickdevice\" id=\"pickdevice\" onchange=\"document.getdetails.submit\" >\n");
// Fetch the results from the database.
$rcount = 0;
while (($Row2 = ibase_fetch_row($Result))) {
...
*/
{
	...
	whm2buttons("Accept","../mainmenu.php", "N","Back_50x100.gif","Back","accept.gif","N");
	$alt = "Allocate Orders";
	//echo ("<tr>");
	echo ("<td>");
...
*/
		echo('SRC="/icons/whm/allocatepicks.gif" alt="' . $alt . '">');
	}
	echo("</form>");
	echo ("</td>");
	echo ("</tr>");
