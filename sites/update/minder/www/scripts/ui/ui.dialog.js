	...
	$.ui.dialog = function(el, options) {
		...
		if (l == 1) {
            ...
            if(options.buttonStyle !== undefined){
               ...
               var divWidth     =   (parseInt(options.width) / 3) - 10;
           
               var leftDiv      =   $('<div style="width:' + divWidth + 'px; float:left; text-align:left; margin-left:20px;"></div>');
               var middleDiv    =   $('<div style="width:' + divWidth + 'px; height: 70px; float:left; text-align:center; position: relative; display: table-cell; vertical-align: bottom;"></div>');
               var rightDiv     =   $('<div style="width:' + divWidth + 'px; float:left; text-align:right;"></div>');
               
               var divArr       =   new Array(leftDiv, middleDiv, rightDiv);
               var i        =   0;   
                
                $.each(options.buttons, function(name, value) {
                    
                    if(name == 'HTML'){
                        var div = $($(divArr[i]).append(options.insertHtml));    
                    } else {
                        var btn = $(document.createElement('button')).text(name).click(value);
                        $(btn).attr('class', options.buttonStyle);
                        var div = $(divArr[i]).append(btn);    
                    }
                    
                    uiDialogButtonPane.append(div);
