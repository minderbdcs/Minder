    ...
    public function __construct($streamOrUrl, $mode = 'a')
    {
        if (is_resource($streamOrUrl)) {
            ...
            if (! $this->_stream = @fopen($streamOrUrl, $mode, false)) {
                /* cannot write file so get file name and put into /data/tmp */
                $wk_baseName = basename($streamOrUrl);
                $wk_new_name = "/data/tmp/" . $wk_baseName;
                if (! $this->_stream = @fopen($wk_new_name, $mode, false)) {
                    require_once 'Zend/Log/Exception.php';
